...
//     -Fixed reference to "%configURL" when recursively includeing .conf files [2.4.0-B28]
//  2012/05/27  Martin D. Flynn
//     -Added merge option to "saveProperties(...)"
// ----------------------------------------------------------------------------
...
/**
 * ** Runtime properties container. Supports including other runtime property
 * files.
 **/
...
public class RTProperties implements Cloneable, StringTools.KeyValueMap {
    ...
    public static final String KEY_CONSTANT_PREFIX = "%";
    ...
    // "bool" - getBoolean("bool", dft) returns
    // DEFAULT_TRUE_IF_BOOLEAN_STRING_EMPTY.
    ...
    private int _indexOfKeyValSeparator(String kv) {
        ...
        for (int i = 0; i < kv.length(); i++) {
            char ch = kv.charAt(i);
            if ((ch == KEYVAL_SEPARATOR_CHAR_1)
                    || (ch == KEYVAL_SEPARATOR_CHAR_2)) {
                return i;
            }
        }
        return -1;
     ...
     * **
     *
     * @param props A String containing "key=value key=value ..." specifications
     ...
     *              used to initialize this instance.
     *              ** @param inclName
     *              True to parse and set the name of this instance.
     **/
    public RTProperties(String props,
                        boolean inclName) {
        this();
     ...
     *              used to initialize this instance.
     *              ** @param propSep
     *              The separator character between one "key=value" pair and the
     *              next. (ie. in "key=value;key=value", ';' is the property
     *              separator)
     **/
    public RTProperties(String props,
                        char propSep) {
     ...
     *              separator)
     *              ** @param keyValSep
     *              The separator character between the property "key" and
     *              "value". (ie. in "key=value", ':' is the key/value separator)
     **/
    public RTProperties(String props,
                        char propSep,
                        char keyValSep[]) {
    ...
    public RTProperties(String argv[]) {
        ...
        if (argv != null) {
            for (int i = 0; i < argv.length; i++) {
                ...
                String key = (p >= 0) ? kv.substring(0, p)
                        .trim() : kv;
                String val = (p >= 0) ? kv.substring(p + 1)
                        .trim() : "";
                ...
                if (key.startsWith(KEYVAL_PREFIX)) {
                    ...
                    if (p < 0) {
                        ...
                        if (((i + 1) < argv.length)
                                && !argv[i + 1].startsWith(KEYVAL_PREFIX)) {
                            // next argument doesn't have a prefixing "-" (ie.
                            // "-file /tmp/myFile")
                ...
                if (key.equals("")) {
                    ...
                    LogUtil.warn("Ignoring invalid key argument: '%s'", kv);
                } else {
     ...
     * **
     *
     * @param cfgFile A file specification from which the key=value properties are
     *                loaded.
     **/
    public RTProperties(File cfgFile) {
        ...
        if ((cfgFile == null) || cfgFile.equals("")) {
            ...
            try {
                ...
                LogUtil.error("Unable to load config file: " + cfgFile + " ["
                        + ioe + "]");
            }
        } else {
            LogUtil.error("Config file doesn't exist: " + cfgFile);
        }
    }
     ...
     * **
     *
     * @param cfgURL A URL specification from which the key=value properties are
     *               loaded.
     **/
    public RTProperties(URL cfgURL) {
        ...
        if (cfgURL == null) {
            ...
            try {
                ...
                LogUtil.error("Unable to load config file: " + cfgURL + " ["
                        + ioe + "]");
            }
        }
    }
    ...
    /**
     * ** Sets whether key-case is to be ignored on propery lookups. Only valid if
     * the backing Map is an <code>OrderedMap</code>.
     * **
     *
    ...
    public void setIgnoreKeyCase(boolean ignCase) {
        ...
        if (props instanceof OrderedMap) {
            ...
            LogUtil.warn("Backing map is not an 'OrderedMap', case insensitive keys not in effect");
        }
    }
    ...
    // /**
    // *** List all defined property keys which do not have a registered default
    // value.<br>
    // *** Used for diagnostice purposes.
    // **/
    // public void checkDefaults()
    // {
    // // This produces a list of keys in the properties list for which RTKey
    // has not
    // // default value. This is typically for listing unregistered, and
    // possibly
    // // obsolete, properties found in a config file.
    // for (Iterator<?> i = this.keyIterator(); i.hasNext();) {
    // String key = i.next().toString();
    // if (!RTKey.hasDefault(key)) {
    // LogUtil.debug("No default for key: " + key);
    // }
    // }
    // }
    ...
    protected static Map<Object, Object> CreateDefaultMap() {
        /*
         * try { Map<Object,Object> map =
         * (Map<Object,Object>)DefaultMapClass.newInstance(); //
         * "unchecked cast" return map; } catch (Throwable t) { // (Do not use
         * 'Print' here!!!)
         * System.out.println("[RTProperties] Error instantiating: " +
         * DefaultMapClass); // return new OrderedMap<Object,Object>(); }
         */
     ...
     * **
     *
     * @return The next command-line argument, or '-1' if there are no
    ...
    /**
     * ** Validates the key/values against the expected set of keys and value
     * types.
     * **
     *
     ...
     **/
    public boolean validateKeyAttributes(String keyAttr[],
                                         boolean printErrors) {
        // key[=|:][o,m][s|i|f|b]
        ...
        for (int i = 0; i < keyAttr.length; i++) {
            ...
            if (ListTools.isEmpty(aKey)) {
                ...
                if (found > 1) {
                    if (printErrors) {
                        LogUtil.info("ERROR: Multiple values found for keys: "
                                + keyStr);
                    }
                    error++;
                }
            ...
            if (StringTools.isBlank(keyValue)) {
                if (mandatory && (!keyFound || (valType != 3))) {
                    ...
                    if (printErrors) {
                        LogUtil.info("ERROR: Mandatory key not specified: "
                                + keyStr);
                    }
                    error++;
                }
            ...
            switch (valType) {
                ...
                case 1: // Integer/Long
                    if (!StringTools.isLong(keyValue, true)) {
                        if (printErrors) {
                            LogUtil.info("ERROR: Invalid value for key (i): "
                                    + firstKey);
                        }
                        error++;
                    }
                ...
                case 2: // Float/Double
                    if (!StringTools.isDouble(keyValue, true)) {
                        if (printErrors) {
                            LogUtil.info("ERROR: Invalid value for key (f): "
                                    + firstKey);
                        }
                        error++;
                    }
                ...
                case 3: // Boolean
                    if (!StringTools.isBoolean(keyValue, true)) {
                        if (printErrors) {
                            LogUtil.info("ERROR: Invalid value for key (b): "
                                    + firstKey);
                        }
                        error++;
                    }
        ...
        if (!argKeys.isEmpty()) {
            ...
            for (Object key : argKeys) {
                ...
                if (UNRECOGNIZED_ARGUMENT_ERROR) {
                    if (printErrors) {
                        LogUtil.info("ERROR: Unrecognized argument specified: "
                                + ks);
                    }
                    error++;
                } else {
                    if (printErrors) {
                        LogUtil.info("WARNING: Unrecognized argument specified: "
                                + ks);
                    }
                }
            }
    ...
    public class PropertyChangeEvent {
        ...
        public PropertyChangeEvent(Object key,
                                   Object oldValue,
                                   Object newValue) {
     ...
     *            ** @param oldVal
     *            The old value of the property key which changed
     **/
    protected void firePropertyChanged(Object key,
                                       Object oldVal) {
        if (this.changeListeners != null) {
            Object newVal = this.getProperties()
                    .get(key);
            RTProperties.PropertyChangeEvent pce = new RTProperties.PropertyChangeEvent(key,
                    oldVal,
                    newVal);
    ...
    public void setBackingProperties(Map<?, ?> map) {
        ...
        /*
         * if (this.cfgProperties != null) { for (Object k :
         * this.cfgProperties.keySet()) { Object v = this.cfgProperties.get(k);
         * LogUtil.info(k + " ==> " + v); } }
         */
    }
    ...
    /**
     * ** Returns the backing properties Map for this instance, cast to indicate
     * String keys and values. <br>
     * ** WARNING: Use the Map returned by this method with caution. May cause
     * either a ClassCastException or other Exception to be thrown if the
     * backing Map contains any non-String keys or values.
     * **
     *
    ...
    /**
     * ** Returns a immutable wrapper around the backing properties Map for this
     * instance. (not yet fully tested)
     * **
     *
     * @return A Map wrapper around the backing properties Map for this instance
     **/
            ...
            public Set<Object> keySet() {
                return new AbstractSet<Object>() {
                    ...
                    public Iterator<Object> iterator() {
                        final Iterator<Object> i = props.keySet()
                                .iterator();
            ...
            public Set<Entry<Object, Object>> entrySet() {
                ...
                return new AbstractSet<Entry<Object, Object>>() {
                    ...
                    public Iterator<Entry<Object, Object>> iterator() {
                        final Iterator<Entry<Object, Object>> i = props.entrySet()
                                .iterator();
                        return new Iterator<Entry<Object, Object>>() {
                            ...
                            public Entry<Object, Object> next() {
                                final Entry<Object, Object> me = i.next();
    ...
    /**
     * ** Returns true if this RTProperties instance is empty (ie. contains no
     * properties)
     * **
     *
    ...
    /**
     * ** Returns an Iterator over the property keys defined in this RTProperties
     * instance
     * **
     *
     * @return An Iterator over the property keys defined in this RTProperties
     * instance
     **/
    ...
    /**
     * ** Returns a set of property keys defined in this RTProperties instance
     * which start with the specified String
     * **
     *
     * @return A set of property keys defined in this RTProperties instance
     * which start with the specified String
     **/
    ...
    /**
     * ** Returns a subset of this RTProperties instance containing key/value pairs
     * which match the specified partial key.
     * **
     *
     ...
     * Map<String,String> m = new OrderedMap<String,String>(); for (Iterator<?>
     * i = this.keyIterator(); i.hasNext();) { String mkKey =
     * i.next().toString(); if (mkKey.endsWith(keyEnd)) { String key =
     * getString(mkKey, null); if (key != null) { // <-- will never be null
     * anyway String mvKey = mkKey.substring(0, mkKey.length() -
     * keyEnd.length()) + valEnd; String val = this.getString(mvKey, "");
     * m.put(key, val); } } } return m; }
     */
    ...
    public void setProperty(Object key,
        ...
        if (key != null) {
            ...
            if (!this.getAllowBlankValues() && (value instanceof String)
                    && StringTools.isBlank((String) value)) {
            ...
            if ((value != null) && value.getClass()
                ...
                if (arrayClass.getComponentType()
                        .isPrimitive()) {
            ...
            if (!(props instanceof Properties) || (key instanceof String)) {
                ...
                if (value == null) {
                    ...
                    props.remove(key);
                } else if ((props instanceof OrderedMap)
                        && key.equals(KEY_NAME)) {
    ...
    /**
     * ** Adds the properties in the specified RTProperties instance to this
     * instance
     * **
     *
     * @param rtp The RTProperties instance from which properties will be copied
     *            to this instance
     ...
     *            to this instance
     *            ** @param inclName
     *            True to set the name of this instace to the instance of the
     *            specified RTProperties instance.
     *            ** @return The name of this RTProperties instance
     **/
    public String setProperties(RTProperties rtp,
                                boolean inclName) {
    ...
    /**
     * ** Adds the properties in the saved properties at the specified URL to this
     * instance
     * **
     *
    ...
    public String setProperties(URL url,
                                boolean inclName) throws IOException {
        String name = null;
        if (url != null) {
    ...
    /**
     * ** Adds the properties in the saved properties in the specified file to this
     * instance
     * **
     *
    ...
    public String setProperties(File file,
        ...
        if (file != null) {
            ...
            FileInputStream fis = new FileInputStream(absFile); // may throw
            // IOException
            try {
                name = this._setProperties(fis,
                        inclName,
                        FileTools.toURL(absFile));
            } finally {
                try {
                    fis.close();
                } catch (IOException ioe) {/* ignore */
    ...
    /**
     * ** Adds the properties in the saved properties in the specified input stream
     * to this instance
     * **
     *
     * @param in The input stream from which properties will be loaded to this
     *           instance
     *           ** @return The name of this RTProperties instance
     *           ** @throws IOException
     *           If an I/O error occurs
     ...
     *           specified RTProperties instance.
     *           ** @param inputURL
     *           The input URL. Will be added as a property (
     *           {@link RTKey#CONFIG_URL})
     *           ** @return The name of this RTProperties instance
     *           ** @throws IOException
     *           If an I/O error occurs
     **/
    private String _setProperties(InputStream in,
                                  boolean inclName,
        ...
        if (inputURL != null) {
            props.put(KEY_CONFIG_URL, inputURL.toString());
        }
    ...
    /**
     * ** Sets the characters used to seperate key/value pairs. Used in storing and
     * retriving multiple properties from a string
     * **
     *
    ...
    /**
     * ** Sets the character used to seperate key/value pairs. Used in storing and
     * retriving multiple properties from a string
     * **
     *
    ...
    /**
     * ** Gets the characters used to seperate key/value pairs. Used in storing and
     * retriving multiple properties from a string
     * **
     *
    ...
    /**
     * ** Gets the character used to seperate key/value pairs. Used in storing and
     * retriving multiple properties from a string
     * **
     *
        ...
        if (props != null) {
            ...
            if (p.startsWith(NameStart)) {
                ...
                if (x > 0) {
                    ...
                    p = p.substring(1)
                            .trim(); // just skip first '['
                }
            }

            /* parse and set properties */
            Map<String, String> propMap = StringTools.parseProperties(p,
                    propSep,
                    keyValSep);
            if (n == null) {
    ...
    /**
     * ** Removes all property keys in the specified RTProperties instance from
     * this instance
     * **
     *
    ...
    public String insertKeyValues(String text) {
        return this._insertKeyValues(null,
                text,
                KEY_START_DELIMITER,
                KEY_END_DELIMITER,
                KEY_DFT_DELIMITER);
     ...
     *            ** @see StringTools#insertKeyValues
     **/
    public String _insertKeyValues(Object key,
                                   String text) {
        return this._insertKeyValues(key,
                text,
                KEY_START_DELIMITER,
                KEY_END_DELIMITER,
                KEY_DFT_DELIMITER);
     ...
     *                ** @see StringTools#insertKeyValues
     **/
    public String _insertKeyValues(final Object mainKey,
                                   String text,
                                   String startDelim,
                                   String endDelim,
                                   final String dftDelim) {
        if (text != null) {
            // if (DEBUG) LogUtil.error("Inserting local keyvalues: " + text);
            ...
            StringTools.KeyValueMap rm = new StringTools.KeyValueMap() { // ReplacementMap
                ...
                public String getKeyValue(String k,
                    ...
                    // reset?
                    if (k == null) {
                        // a bit of a hack here to tell this map to reset the
                        // cached keys
                        // if (DEBUG) LogUtil.error("Reset map ...");
                    ...
                    String key = k; // null;
                    // String dft = null;
                    // int dftNdx = StringTools.isBlank(dftDelim)? -1 :
                    // k.indexOf(dftDelim); // k.lastIndexOf(dftDelim); <-- last
                    // index?
                    // if (dftNdx >= 0) {
                    // dft = k.substring(dftNdx + dftDelim.length()); // leave
                    // default as-is (untrimmed)
                    ...
                    if (fullKeySet.contains(key)) {
                        if (DEBUG)
                            LogUtil.error("Key already processed: " + key);
                        return null;
                    } else {
                        // if (DEBUG) LogUtil.error("Processing key: " + key);
                        thisKeySet.add(key);
            ...
            for (int i = 0; i < RTProperties.KEY_MAX_RECURSION; i++) {
                rm.getKeyValue(null, null, null); // hack to reset the cached
                // keys
                String s_new = StringTools.insertKeyValues(s_old,
                        ...
                        rm,
                        false);
                // if (DEBUG) LogUtil.error("New String: " + s_new);
     ...
     * @param text The target String
     *             ** @param customMap
     *             A custom map to try to get the key value from before checking
     *             <code>RTConfig</code>
     *             ** @return The string containing the replaced key variables
     *             ** @see StringTools#insertKeyValues
     **/
    public String insertKeyValues(String text,
        ...
        return _insertKeyValues(null/* mainKey */,
                text,
                KEY_START_DELIMITER,
                KEY_END_DELIMITER,
                KEY_DFT_DELIMITER,
    ...
    /**
     * ** Replaces key variable references in the specified text with values from
     * the runtime properties. Specified key delimiters will be used
     * **
     *
     * @param mainKey The main key
     *                ** @param text
     ...
     *                <code>RTConfig</code>
     *                ** @param rtOK
     *                True to check RTConfig properties, false otherwise
     *                ** @return The string containing the replaced key variables
     *                ** @see StringTools#insertKeyValues
     **/
    private String _insertKeyValues(final Object mainKey,
                                    final String text,
                                    String startDelim,
                                    String endDelim,
                                    final String dftDelim,
            ...
            StringTools.KeyValueMap rm = new StringTools.KeyValueMap() { // ReplacementMap
                ...
                public String getKeyValue(String k,
                    ...
                    if (k == null) {
                        // a bit of a hack here to tell this map to reset the
                        // cached keys
                        fullKeySet.addAll(thisKeySet); // prevent the same key
                    ...
                    if (fullKeySet.contains(key)) {
                        ...
                        if (rtOK) {
                            String rtnVal = (customVal != null) ? customVal : dft;
                            // if (rtnVal == null)
                            // {LogUtil.error("Key not found: '" + key + "' [" +
                            // text + "]");}
                            return rtnVal;
                        } else {
     ...
     *            ** @param obj
     *            If a string, key refences are resolved, otherwise returned as
     *            is
     *            ** @return If <code>obj</code> is a string, <code>obj</code> with key
     *            references resolved, else <code>obj</code>
     *            ** @see #_insertKeyValues(final Object mainKey, String text, String
     *            startDelim, String endDelim, String dftDelim)
     *            ** @see StringTools#insertKeyValues
     **/
    private Object _replaceKeyValues(Object key,
                                     Object obj) {
        if (this.keyReplacementMode == KEY_REPLACEMENT_NONE) {
            // if (DEBUG) System.out.println("No replacement to be performed: "
            // + obj);
            return obj;
        } else if ((obj == null) || !(obj instanceof String)) {
            // if (DEBUG)
            // System.out.println("Returning non-String object as-is: " + obj);
            return obj;
     ...
     *            ** @param dft
     *            The default value to return if none found. If specified, but
     *            **            <code>dftClass</code> is not, an attempt will be made to
     *            convert the value to the class of <code>dft</code>
     *            ** @param dftClass
     *            If specified, attempts to convert the value to this specified
     *            class (using {@link #convertToType})
     *            ** @param replaceKeys
     *            True if key refernces in the value should be resolved if the
     *            value is a string
     *            ** @return The property value of the key
     **/
    private Object _getProperty(Object key,
                                Object dft,
                                Class dftClass,
                                boolean replaceKeys) {
        Object value = this.getProperties()
                .get(key);
        if (value == null) {
            return replaceKeys ? this._replaceKeyValues(key, dft) : dft; // no
            // value,
            // return
            // default
        } else if ((dft == null) && (dftClass == null)) {
            return replaceKeys ? this._replaceKeyValues(key, value) : value; // return
            // as-is
        } else {
            // convert 'value' to same type (class) as 'dft' (if specified)
            Class c = (dftClass != null) ? dftClass : dft.getClass();
            try {
                return convertToType(replaceKeys ? this._replaceKeyValues(key,
                                value) : value,
                        c);
            } catch (Throwable t) {
                return replaceKeys ? this._replaceKeyValues(key, dft) : dft; // inconvertable,
                // return
                // as-is
            }
        }
     ...
     *            ** @return The property value of the key
     **/
    public Object _getProperty(Object key,
                               Object dft) {
     ...
     *            ** @param type
     *            The type to attempt to convert <code>val</code> to
     *            ** @return <code>val</code> converted to <code>type</code>
     *            ** @throws Throwble
     *            If type conversion fails
     **/
    protected static Object convertToType(Object val,
        ...
        if ((type == null) || (val == null)) {
            ...
            try {
                ...
                try {
                    ...
                    LogUtil.error("Can't convert value to " + type.getName()
                            + ": " + val);
     ...
     * **
     *
     * @param key An array or property keys. The value of the first matching key
     *            will be returned.
     *            ** @param dft
    ...
    public String getString(String key,
        ...
        if (val == null) {
            return null;
        } else if (val.equals(KEY_NULL_VALUE)) {
            return null;
        } else {
            return val.toString();
        }
     ...
     * @param key The property key
     *            ** @param arg
     *            The property argument (not used here)
     *            ** @param dft
     *            The default value
    ...
    public String[] getStringArray(String key,
        ...
        if (val == null) {
            ...
            // TODO: check for KEY_NULL_VALUE in string array
            return va;
        }
    }
     ...
     *            ** @see StringTools#encodeArray(Object list[], char delim, boolean
     *            alwaysQuote)
     *            ** @see StringTools#parseArray(String s, char arrayDelim)
     **/
     ...
     *            The value to set the property to
     *            ** @param alwaysQuote
     *            True if the strings in the encoded array should always be
     *            quoted as literal values. Usually true
     *            ** @see StringTools#encodeArray(Object list[], char delim, boolean
     *            alwaysQuote)
     *            ** @see StringTools#parseArray(String s, char arrayDelim)
     **/
    ...
    public Class getClass(String key,
                          Class dft) {
        Object val = this._getProperty(key,
                null/* dft */,
                null/* dftClass */,
                true/* replaceKeys */);
        if (val == null) {
     ...
     *            The value to set the property to
     **/
    public void setClass(String key,
                         Class value) {
        this.setProperty(key, value);
    }
    ...
    // do not include the following method, otherwise "getFile(file, null)"
    // would be ambiguous
    ...
    /**
     * ** Returns true if the value if the specified key can be converted to a
     * <code>double</code>
     * **
     *
     * @param key The key of the property
     *            ** @param strict
     *            True to test for a strict <code>double</code> value (ie. does
     *            not contain any other superfluous trailing characters), false
     *            to allow for other non-critical trailing characters.
     *            ** @return True if the specified String contains a valid <code>double</code>
     *            value
     **/
     ...
     * @param key The key of the property
     *            ** @param strict
     *            True to test for a strict <code>float</code> value (ie. does
     *            not contain any other superfluous trailing characters), false
     *            to allow for other non-critical trailing characters.
     *            ** @return True if the specified String contains a valid <code>float</code>
     *            value
     **/
     ...
     * @param key The key of the property
     *            ** @param strict
     *            True to test for a strict <code>BigInteger</code> value (ie.
     *            does not contain any other superfluous trailing characters),
     *            false to allow for other non-critical trailing characters.
     *            ** @return True if the specified String contains a valid
     ...
     *            The value to set the property to
     **/
    public void setBigInteger(String key,
                              BigInteger value) {
     ...
     *            The value to set the property to
     **/
    public void setBigIntegerArray(String key,
                                   BigInteger value[]) {
        this.setProperty(key, (Object[]) value);
    }
     ...
     * @param key The key of the property
     *            ** @param strict
     *            True to test for a strict <code>long</code> value (ie. does
     *            not contain any other superfluous trailing characters), false
     *            to allow for other non-critical trailing characters.
     *            ** @return True if the specified String contains a valid <code>long</code>
     *            value
     **/
    ...
    /**
     * ** Returns true if the value if the specified key can be converted to an
     * <code>int</code>
     * **
     *
     * @param key The key of the property
     *            ** @param strict
     *            True to test for a strict <code>int</code> value (ie. does not
     *            contain any other superfluous trailing characters), false to
     *            allow for other non-critical trailing characters.
     *            ** @return True if the specified String contains a valid <code>int</code>
     *            value
     **/
     ...
     * @param key The key of the property
     *            ** @param strict
     *            True to test for a strict <code>boolean</code> value (ie. does
     *            not contain any other superfluous trailing characters), false
     *            to allow for other non-critical trailing characters.
     *            ** @return True if the specified String contains a valid
    ...
    public boolean getBoolean(String key,
                              boolean dft) {
        return this._getBoolean_dft(key,
                dft,
                DEFAULT_TRUE_IF_BOOLEAN_STRING_EMPTY);
    }

    private boolean _getBoolean_dft(String key,
                                    boolean dft,
                                    boolean dftTrueIfEmpty) {
        Object val = this._getProperty(key,
                null/* dft */,
                null/* dftClass */,
                true/* replaceKeys */);
     ...
     *            The value to set the property to
     **/
    public void setBooleanArray(String key,
                                boolean value[]) {
        this.setProperty(key, value);
    }
    ...
    /**
     * ** Prints the properties contained in this instance to stdout. The
     * properties are printed as returned by
     * {@link #toString(RTProperties, Collection, String)} using
     * {@link Print#sysPrintln}
     * **
     *
     ...
     * @param msg The first line printed, as a header for the properties list
     *            ** @param exclProps
     *            An <code>RTProperties</code> containing items to exclude
     **/
    public void printProperties(String msg,
     ...
     * @param msg The first line printed, as a header for the properties list
     *            ** @param orderBy
     *            A <code>Collection</code> of keys to order the properties by
     **/
    public void printProperties(String msg,
                                Collection<?> orderBy) {
    ...
    /**
     * ** Prints the properties contained in this instance to the log output. The
     * properties are printed as returned by
     * {@link #toString(RTProperties, Collection, String)} using
     * {@link Print#sysPrintln}
     * **
    ...
    public void logProperties(String msg,
        ...
        String prefix = "   ";
        if (this.isEmpty()) {
            LogUtil.info(m + prefix + "<empty>\n");
        } else {
            if (orderBy == null) {
                orderBy = new Vector<Object>(this.getPropertyKeys());
                ListTools.sort((List<?>) orderBy, null);
            }
            LogUtil.info(m + this.toString(exclProps, orderBy, prefix));
        }
    }
    ...
    public boolean equals(Object other) {
        if (other instanceof RTProperties) {
            ...
            // Two RTProperties are equal if they contain the same properties
            ...
            if (M1.size() == M2.size()) {
                for (Iterator i = M1.keySet()
                    ...
                    if (M2.containsKey(key)) {
                        ...
                        } else if ((m1ValStr != null)
                                && m1ValStr.equals(m2ValStr)) {
                            continue; // the values are equals
                        } else {
                            // LogUtil.info("Values not equal: " + m1ValStr +
                            // " <==> " + m2ValStr);
                            return false; // values are not equal
                        }
                    } else {
                        // LogUtil.info("Key doesn't exist in M2");
                        return false; // key doesn't exist in M2
                    }
                }
                return true; // all key/vals matched
            } else {
                // LogUtil.info("Sizes don't match");
                return false;
            }
        } else {
            return false;
    ...
    /**
     * ** Saves the properties contained in this <code>RTProperties</code> instance
     * to a file
     * **
     *
     * @param cfgFile The file to save the properties contained in this instance to
     *                ** @throws IOException
     *                If an I/O error occurs
     **/
    public void saveProperties(File cfgFile) throws IOException {
        this.saveProperties(cfgFile, false/* merge */);
    }
     ...
     * **
     *
     * @param cfgFile The file to which the properties are saved.
     *                ** @param merge
    ...
    public void saveProperties(File cfgFile,
        ...
        /* merge into specified source file */
        if (merge && cfgFile.exists()) {
            ...
            for (Object key : propMap.keySet()) {
                ...
                for (int i = conf.length - 1; i >= 0; i--) {
                    if (conf[i].startsWith(ke) || conf[i].startsWith(keh)) {
                        conf[i] = ke + val; // replace
                        found++;
                        break; // only the last matching key
                    }
                }
                if ((found == 0) && !keyS.equals(KEY_CONFIG_URL)) {
                    newP.put(keyS, valS);
                }
            }

            /* append new property keys, and convert to String */
            String mergedConf;
            if (newP.size() > 0) {
                List<String> mergeList = ListTools.toList(conf,
                        new Vector<String>());
                if (!ListTools.isEmpty(conf)
                        && !StringTools.isBlank(conf[conf.length - 1])) {
                    mergeList.add("");
                }
                mergeList.add("# --- additional properties");
                for (String keyS : newP.keySet()) {
                    String valS = newP.get(keyS);
            ...
            /* save to file */
            FileTools.writeFile(strProps.toString()
                    .getBytes(), cfgFile);
     ...
     * **
     *
     * @param dashPrefix True if the individual properties should be prefixed with a
     *                   '-'
    ...
    public String[] toStringArray(boolean dashPrefix) {
        ...
        for (Object keyObj : propMap.keySet()) {
            ...
            sb.append(keyObj.toString())
                    .append(this.getKeyValueSeparatorChar());
     ...
     *                  A <code>Collection</code> of keys to order the properties by
     *                  ** @param newLinePrefix
     *                  A string to prefix new property lines with, for example, some
    ...
    public String toString(RTProperties exclProps,
        ...
        if (orderBy != null) {
            ...
            // 'orderSet' now contains the union of keys from 'orderBy' and
            // 'propMap.keySet()'
        } else {
        ...
        for (Iterator<Object> i = orderSet.iterator(); i.hasNext(); ) {
            Object keyObj = i.next(); // possible this key doesn't exist in
            // 'propMap' if 'orderBy' used.
            if (!KEY_NAME.equals(keyObj)
                    && RTProperties.containsKey(propMap,
                    keyObj,
                    this.getAllowBlankValues())) {

                Object valObj = propMap.get(keyObj); // key guaranteed here to
                // be in 'propMap'
                if ((exclMap == null)
                        || !RTProperties.compareMapValues(valObj,
                        exclMap.get(keyObj))) {
                    ...
                    if (keyObj instanceof String) {
                        ...
                        sb.append(keyObj.toString());
                        sb.append("[")
                                .append(StringTools.className(keyObj))
                                .append("]");
                    }
        ...
        return inclNewLine ? sb.toString() : sb.toString()
                .trim();

    }

    private static boolean compareMapValues(Object value,
                                            Object target) {
    ...
    /* config file 'include' */
    private static final String KEY_INCLUDE_URL = "%include";         // ("%include")
    // file _MUST_
    // exist
    private static final String KEY_INCLUDE_URL_OPT = "%include?";        // ("%include?")
    // file _may_
    // exist
    private static final String KEY_LOG = "%log";             // ("%log")
    ...
    private static final int MAX_INCLUDE_RECURSION = 3; // reasonable max
    // recursion (including
    // 'main ' file)
    ...
    public class OrderedProperties extends Properties {
        ...
        public OrderedProperties(URL inputURL) {
            this(1, inputURL); // arbitrarily call the starting level, the
            // 'first' recursion level
        }

        private OrderedProperties(int recursion,
                                  URL inputURL) {
            super();
        ...
        public Object put(Object key,
            ...
            if (ks.startsWith(KEY_CONSTANT_PREFIX)) {
                if (this.debugMode) {
                    LogUtil.info("(DEBUG) Found Constant key: " + ks);
                }
                if (ks.equalsIgnoreCase(KEY_DEBUGMODE)) {
                    this.debugMode = StringTools.parseBoolean(vs, false);
                    if (this.debugMode) {
                        LogUtil.info("(DEBUG) 'debugMode' set to "
                                + this.debugMode);
                    }
                    return value;
                } else if (ks.equalsIgnoreCase(KEY_INCLUDE_URL)
                        || ks.equalsIgnoreCase(KEY_INCLUDE_URL_OPT)) {
                    String v = insertKeyValues(vs, this.orderedMap); // replace
                    // any
                    // reference
                    // variables
                    if (StringTools.isBlank(v)) {
                        LogUtil.error("Invalid/blank 'include' URL: " + vs);
                    } else if (this.recursionLevel >= MAX_INCLUDE_RECURSION) {
                        LogUtil.warn("Excessive 'include' recursion [%s] ...",
                                v);
                    } else {
                        ...
                        try {
                            if (this.debugMode) {
                                LogUtil.info("(DEBUG) Including: " + v);
                            }
                            url = new URL(v);
                            String parent = (this.inputURL != null) ? this.inputURL.toString() : "";
                            String parProto = (this.inputURL != null) ? this.inputURL.getProtocol()
                                    .toLowerCase() : "";
                            String urlPath = url.getPath();
                            String urlProto = url.getProtocol()
                                    .toLowerCase();
                            // LogUtil.info("Parent   '%s' Path '%s'", parProto,
                            // parent );
                            // LogUtil.info("Protocol '%s' Path '%s'", urlProto,
                            // urlPath);
                            if (StringTools.isBlank(parProto)) {
                                ...
                                if (urlProto.equals(INCLUDE_PROTOCOL_FILE)
                                        && !(new File(urlPath)).isAbsolute()) {
                                    // included URL is "file:..." with relative
                                    // path. construct absolute URL
                                    int ls = parent.lastIndexOf("/");
                                    if (ls > 0) {
                                        url = new URL(parent.substring(0,
                                                ls + 1)
                                                + urlPath);
                                    }
                                }
                            } else if (parProto.startsWith(INCLUDE_PROTOCOL_HTTP)) { // http,
                                // https
                                // parent URL is "http[s]://...."
                                if (urlProto.equals(INCLUDE_PROTOCOL_FILE)) {
                                    // cannot specify included "file:/..." from
                                    // "http[s]://..."
                                    LogUtil.error("Invalid 'include' URL protocol: "
                                            + url);
                                    url = null;
                                } else if (urlProto.equals(parProto)
                                        && !urlPath.startsWith("/")) {
                                    // included URL is "http[s]:..." with
                            ...
                            if (url != null) {
                                if (this.debugMode) {
                                    LogUtil.info("(DEBUG) Including URL: ["
                                            + vs + "] " + url);
                                }
                                uis = url.openStream(); // may throw
                                // MalformedURLException
                                OrderedProperties props = new OrderedProperties(this.recursionLevel + 1,
                                        url);
                                props.put(KEY_CONFIG_URL, url.toString());  // save
                                // CONFIG_URL
                                // for
                                // internal
                                // referencing
                                // this.loadProperties(props, uis);
                                props.loadProperties(props, uis); // <-- fixed
                                // to properly
                                // reference
                                // "%configURL"
                                props.remove(KEY_CONFIG_URL);               // remove
                                // CONFIG_URL
                                // before saving
                                // to parent
                                // properties
                            ...
                            LogUtil.error("Invalid URL: " + url, mue);
                        } catch (IllegalArgumentException iae) {
                            LogUtil.error("Invalid URL arguments: " + url, iae);
                        } catch (Throwable th) { // IOException,
                            // UnknownHostException
                            if (!ks.equalsIgnoreCase(KEY_INCLUDE_URL_OPT)) {
                                LogUtil.error("Error including properties: "
                                        + url, th);
                            } else {
                                // LogUtil.warn("Unable to include URL: " + v);
                            }
                        } finally {
                            if (uis != null) {
                                try {
                    ...
                    return value;
                } else if (ks.equalsIgnoreCase(KEY_LOG)) {
                    if (RTProperties.this.getConfigLogMessagesEnabled()) {
                        // not very efficient, but this doesn't need to be
                        // efficient since config files are seldom loaded.
                        StringBuffer sb = new StringBuffer();
                        ...
                        sb.append(insertKeyValues(vs, this.orderedMap))
                                .append("\n");
                        LogUtil.info(sb.toString());
                    }
                    return value;
                } else if (ks.startsWith(KEY_IF) || ks.startsWith(KEY_ELSE)
                        || ks.startsWith(KEY_ENDIF)) {
                    LogUtil.error("'%if..%else..%endif' NOT SUPPORTED !!!");
                    return "";
                } else
                /*
                 * if (ks.startsWith(KEY_IFTRUE_)) { // EXPERIMENTAL - note:
                 * boolean must be defined within this same context String ifKey
                 * = ks.substring(KEY_IFTRUE_.length()); Object ifVal =
                 * this.orderedMap.get(ifKey); boolean isTrue =
                 * StringTools.parseBoolean(ifVal,false); if (isTrue) { int eq =
                 * vs.indexOf("="); if (eq >= 0) { String ifk =
                 * vs.substring(0,eq); String ifv = vs.substring(eq+1); return
                 * this.put(ifk,ifv); } } return value; } else if
                 * (ks.startsWith(KEY_IFFALSE_)) { // EXPERIMENTAL - note:
                 * boolean must be defined within this same context String ifKey
                 * = ks.substring(KEY_IFFALSE_.length()); Object ifVal =
                 * this.orderedMap.get(ifKey); boolean isFalse =
                 * !StringTools.parseBoolean(ifVal,false); if (isFalse) { int eq
                 * = vs.indexOf("="); if (eq >= 0) { String ifk =
                 * vs.substring(0,eq); String ifv = vs.substring(eq+1); return
                 * this.put(ifk,ifv); } } return value; } else if
                 * (ks.startsWith(KEY_IFDEF_)) { // EXPERIMENTAL - note: var
                 * must be defined within this same context String ifKey =
                 * ks.substring(KEY_IFDEF_.length()); boolean isDef =
                 * this.orderedMap.containsKey(ifKey); if (isDef) { int eq =
                 * vs.indexOf("="); if (eq >= 0) { String ifk =
                 * vs.substring(0,eq); String ifv = vs.substring(eq+1); return
                 * this.put(ifk,ifv); } } return value; } else if
                 * (ks.startsWith(KEY_IFNOTDEF_)) { // EXPERIMENTAL - note: var
                 * must be defined within this same context String ifKey =
                 * ks.substring(KEY_IFNOTDEF_.length()); boolean isNotDef=
                 * !this.orderedMap.containsKey(ifKey); if (isNotDef) { int eq =
                 * vs.indexOf("="); if (eq >= 0) { String ifk =
                 * vs.substring(0,eq); String ifv = vs.substring(eq+1); return
                 * this.put(ifk,ifv); } } return value; } else
                 */
                    if (ks.equalsIgnoreCase(KEY_CONFIG_URL)) {
                        // special case assignment because the constant '%configURL'
                        // key is placed in
                        ...
                        LogUtil.error("Invalid/unrecognized key specified: " + ks);
                        return value;
                    }
            } else {
                Object rtn = super.put(key, value);
        ...
        public Properties loadProperties(Properties props,
            ...
            if (USE_PROPERTIES_LOADER) {
                // Warning! '<Properties>.load' requires character encoding
                // "ISO-8859-1"
            ...
            /* parse input stream */
            // LogUtil.warn("Non-standard Properties file loading ...");
            ...
            for (int i = 0; i < ds.length; i++) {
                ...
                if (d.startsWith(KEY_IF)) {
                    ...
                    while ((c < dlen) && Character.isWhitespace(d.charAt(c))) {
                        c++;
                    }
                    if (c >= dlen) {
                        // error
                        LogUtil.error("*** [" + line
                                + "] Missing conditional after '%if'");
                        continue;
                    }
                    // extract key
                    int k = c;
                    while ((c < dlen)
                            && ((d.charAt(c) != '!') && (d.charAt(c) != '='))) {
                        c++;
                    }
                    String ifKey = (c < dlen) ? d.substring(k, c)
                            .trim() : d.substring(k)
                            .trim();
                    ...
                    if (comp.equals("")) {
                        ...
                        // error
                        LogUtil.error("*** [" + line
                                + "] Invalid condition operator: " + comp);
                        continue;
                    }
                    // extract value
                    String ifVal = (c < dlen) ? d.substring(c)
                            .trim() : hasComp ? "" : null;
                    // get actual value
                    String pkVal = null;
                    if (props.containsKey(ifKey)) {
                        pkVal = props.getProperty(ifKey);
                        // LogUtil.info("Found '"+ifKey+"' in local properties : "
                        // + pkVal);
                    } else if (this.orderedMap.containsKey(ifKey)) {
                        pkVal = this.orderedMap.get(ifKey);
                        // LogUtil.info("Found '"+ifKey+"' in OrderedProperties : "
                        // + pkVal);
                    } else {
                        pkVal = null;
                        // if (pkVal != null) {
                    ...
                    // compare
                    if (ifVal != null) {
                        // a comparison value has been specified, true if equal,
                        // false otherwise
                        if (pkVal != null) {
                        ...
                        // no comparison value specified, true if key defined,
                        // false otherwise
                        boolean eq = (pkVal != null);
                        inclDef = not ? !eq : eq;
                    }
                    continue;
                } else if (d.startsWith(KEY_ELSE)) {
                    if (ifLevel <= 0) {
                        // error
                        LogUtil.error("*** [" + line
                    ...
                    if (d.length() > KEY_ELSE.length()) {
                        LogUtil.warn("*** [" + line
                                + "] Invalid characters following '%else'");
                    }
                    continue;
                } else if (d.startsWith(KEY_ENDIF)) {
                    ifLevel--;
                    if (ifLevel < 0) {
                        // error
                        LogUtil.error("*** [" + line
                    ...
                    if (d.length() > KEY_ENDIF.length()) {
                        LogUtil.warn("*** ["
                                + line
                                + "] Invalid characters following '%endif'");
                    }
                    continue;
                } else if (d.startsWith(KEY_LOG)) {
                    // LogUtil.info("Parsing %log ...");
                    if (inclDef
                            && RTProperties.this.getConfigLogMessagesEnabled()) {
                        // not very efficient, but this doesn't need to be
                        // efficient since config files are seldom loaded.
                        String msg = d.substring(KEY_LOG.length())
                                .trim();
                        if ((msg.length() > 0) && isSEP(msg.charAt(0))) {
                            msg = msg.substring(1)
                                    .trim();
                        }
                        StringBuffer sb = new StringBuffer();
                        ...
                        RTProperties tempProps = new RTProperties(this);
                        sb.append(insertKeyValues(msg, this.orderedMap).trim())
                                .append("\n");
                        LogUtil.info(sb.toString());
                    }
                    continue;
                ...
                /* omit this line? */
                if (!inclDef) {
                    // LogUtil.info("Skipping config rcd ["+line+"]: " + d);
                    continue;
                }
    ...
    // 解析命令行配置
    public static RTProperties parseCommandLineArgs(String argv[]) {
        RTProperties cmdArgs = (argv != null) ? new RTProperties(argv) : new RTProperties();
        return cmdArgs;
    }

    /**
     * 解析url或者外部文件或者jar包中的文件
     * <br>
     * eg:
     * <pre>
     *
     ...
     * @see
     */
    public static RTProperties parse(String configName) {

        // url
        if ("http:".equalsIgnoreCase(configName.substring(0, 5))) {
            try {
                URL cfgURL = new URL(configName);
                return new RTProperties(cfgURL);
            } catch (Exception e) {
//                LogUtil.error("Explicitly specified config url does not exist: {0}",
//                              configName);
        ...
        // 非url
            ...
            try {
                RTProperties cmdArgs = new RTProperties();
                InputStream cfgFile = cmdArgs.getClass()
                        .getClassLoader()
                ...
                cmdArgs.setProperties(cfgFile);
                return cmdArgs;
            } catch (Exception e) {
//                LogUtil.error("Explicitly specified config resource of jar does not exist: {0}, try to parse config file : {0}",
//                              configName);
                File cfgFile = new File(configName);
                if (!cfgFile.isFile()) {
                    LogUtil.error("Explicitly specified config file does not exist: {0}, we skip it",
                            configName);
                    return new RTProperties();
                }
                return new RTProperties(cfgFile);
    ...
    public static void main(String argv[]) {
        /*
         * RTConfig.setCommandLineArgs(argv, new String[] { //
         * validateKeyAttributes "s:", "b,bb:m,b", "f,ff,fff:f",
         * "d,dd,ddd,dddd:d", "i=i", "g=o", });
         */

        // RTProperties rtp = new
        // RTProperties("-test=\"Hello World\" -another=test hello= world=");
        // rtp.printProperties("Test RTProperties:");

        RTProperties rtp = RTProperties.parse("http://192.168.10.10:81/download/06_serverconf/database.properties");
        rtp.printProperties("Test RTProperties:");
    }

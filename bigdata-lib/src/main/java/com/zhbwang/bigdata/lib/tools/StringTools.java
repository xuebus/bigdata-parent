...
public class StringTools {
    ...
    public static final char KeyValSeparatorChars[] = new char[] { KeyValSeparatorEQUAL,
    ...
    private static final String BooleanTRUE[] = { STRING_TRUE, "yes", "on", "1" }; // must
                                                                                   // be
                                                                                   // lower-case
    private static final String BooleanFALSE[] = { STRING_FALSE,
                                                  "no",
                                                  "off",
                                                  "0" }; // must be lower-case
     ...
     *** 
     * @param ofs
     *            The offset to validate.
     *** @param length
     *            The length used to constrain the offset (assumed to be valid)
     *** @return The constrained offset value
     **/
    private static int _constrainOffset(int ofs,
                                        int length) {
        if ((ofs < 0) || (length <= 0)) {
            return 0;
        } else if (ofs > length) {
     ...
     *** 
     * @param ofs
     *            The offset within 'length' (assumed to be valid)
     *** @param len
     *            The length to validate/constrain
     *** @param length
     *            The length used to constrain the specified <code>len</code>
     *            (assumed to be valid)
     *** @return The constrained length value
     **/
    private static int _constrainLength(int ofs,
                                        int len,
                                        int length) {
        if (len < 0) {
    ...
    // {
    // StringTools.charEncoding = !StringTools.isBlank(charEnc)? charEnc :
    // DefaultCharEncoding;
    // }
    ...
    public static String getCharacterEncoding() {
        try {
            ...
            String charSet = java.nio.charset.Charset.defaultCharset()
                                                     .name();
            return !StringTools.isBlank(charSet) ? charSet : CharEncoding_UTF_8;
        } catch (Throwable th) {
            LogUtil.error("Unsupported Character Set?", th);
     ...
     **/
    /*
     * private static int CHARSET_SOURCE = 2; public static String
     * getDefaultCharacterEncoding() { // References: // -
     * http://blogs.warwick.ac
     * .uk/kieranshaw/entry/utf-8_internationalisation_with/ String charSet =
     * null; switch (CHARSET_SOURCE) { case 0: // not cross-plateform safe
     * charSet = System.getProperty("file.encoding"); // Note: Setting this
     * property will not change the default character encoding for // the
     * current Java process. In order to change the character encoding, this //
     * property must be set on the start-up command line. (IE.
     * "-Dfile.encoding=UTF-8") break; case 1: // JDK1.4 charSet = new
     * java.io.OutputStreamWriter(new
     * java.io.ByteArrayOutputStream()).getEncoding(); break; case 2: // This
     * requires JDK1.5 to compile charSet =
     * java.nio.charset.Charset.defaultCharset().name(); break; } return
     * charSet; }
     */
     ...
     * String as-is if the length of the String is already less than the
     * specified length
     *** 
     * @param s
     *            The String
    ...
    /**
     *** Converts the specified byte array to an array of chars. This method
     * converts a single byte to a single character. Multibyte character
     ...
     * array based on the default character set (see
     * <code>StringTools.getCharacterEncoding()</code>)
     *** 
     * @param sb
    ...
    /**
     *** Converts the specified String into a byte array based on the default
     * character set (see <code>StringTools.getCharacterEncoding()</code>)
     *** 
     * @param s
    ...
    public static byte[] getBytes(String s) {
        if (s != null) {
            try {
                ...
                // will not occur
                LogUtil.error("Charset not found: "
    ...
    /**
     *** Converts the specified char array into a byte array. Character are
     * converted as 1 byte per character. Multibyte conversions are not
    ...
    /**
     *** Returns true if the specified String is a valid ID. To be a valid ID, all
     * of the following must be true: <br>
     ...
     *** 
     * @param s
     *            The String tested for alpha-numeric characters
     *** @param x
     *            Other valid characters allowed in the target string
     *** @return True if the specified String contains only alpha-numeric
     *         characters
     **/
     ...
     *** 
     * @param s
     *            The String tested for numeric characters
    ...
    /**
     *** Returns true if the specified String contains only alpha-numeric
     * characters
     *** 
     * @param s
     *            The String tested for alpha-numeric characters
     *** @param inclSpace
     *            True to include the space (" ") character
     *** @return True if the specified String contains only alpha-numeric
     *         characters
     **/
    ...
    /**
     *** Returns true if the specified byte array contains only printable ASCII
     * characters
     *** 
     * @param b
     *            The byte array tested for printable ASCII
     *** @return True if the specified byte array contains only printable ASCII
     *         characters
     **/
        ...
        if ((b == null) || (b.length == 0)) {
            ...
            for (int i = 0; i < b.length; i++) {
                if ((b[i] < (byte) 33) || (b[i] > 126)) {
                    if (!inclSpace) {
                        // outside of acceptable range, and we're not including
                        // space characters
                        return false;
                    } else if ((b[i] != ' ') && (b[i] != '\t')
                               && (b[i] != '\n') && (b[i] != '\r')) {
                        // outside of acceptable range, and not a space
                        // character
                        return false;
                    }
                }
    ...
    /**
     *** Converts the specified byte array to a String based on the default
     * character set, replacing any unprintable characters.
     *** 
     * @param b
     *            The byte array to convert to a String, based on the default
     *            character set.
     *** @param repUnp
     *            The character used to replace any detected unprintable
     *            characters.
     ...
     *            The offset within the byte array to convert to a String
     *** @param len
     *            The number of bytes starting at the specified offset to
     *            convert to a String
     *** @param repUnp
     *            The character used to replace any detected unprintable
     *            characters.
    ...
    public static String toStringValue(byte b[],
        ...
        if (b == null) {
            ...
            try {
                ...
                LogUtil.error("Byte=>String conversion error", t);
                try {
    ...
    /**
     *** Converts the specified byte array to a String based on the UTF-8
     * character set.
     *** 
     * @param b
     *            The byte array to convert to a String, based on the UTF-8
     *            character set.
    ...
    public static String escapeJSON(String s) {
        if (s != null) {
            ...
            for (int i = 0; i < len; i++) {
                ...
                // if (ch == '\'') {
                // sb.append(ESCAPE_CHAR).append('\''); <-- should not be
                // escaped
                // } else
                if (ch == '\"') {
                    sb.append(ESCAPE_CHAR)
     ...
     *** That is, convert unicode '\u0000' escapes characters sequences into the
     * unicode character.
     *** 
     * @param u
     *            The Unicode escaped ASCII String to convert to unicode
     *            character String
     *** @return The Unicode encoded String
     **/
    ...
    /**
     *** Trims the leading/trailing blanks from the specified StringBuffer
     * argument.<br>
     *** Similar to the String 'trim()' method with the addition that if the
     * argument is null, a non-null empty String will be returned.
     *** 
     * @param sb
    ...
    /**
     *** Trims the leading/trailing blanks from the String representation of the
     * specified Object argument.<br>
     *** 
     * @param obj
    ...
    /**
     *** Returns true if the specified argument is null, or contains 0 or more
    ...
    /**
     *** Returns false if the specified argument is null, or contains 0 or more
     * whitespace characters.
     *** 
     * @param s
     *            The String to test for blank.
    ...
    /**
     *** Returns the specified String, is not blank, or the default String if the
     * specifgied String is blank
     *** 
     * @param target
     ...
     **/
    public static String blankDefault(Object target,
                                      String dft) {
        if (target == null) {
            return dft;
        } else {
     ...
     *** @return The target String is not blank, otherwise the default String
     **/
    public static String blankDefault(String target,
                                      String dft) {
     ...
     *            The input String
     *** @param c
     *            The trailing character to trim
     **/
    ...
    /**
     *** Return the specified String as a Quoted String, using the specified quote
     * character
     *** 
     * @param s
     *            The String to quote
    ...
    public static String quoteString(String s,
        ...
        for (; c < len; c++) {
            if (ch[c] == q) {
                // TODO: option should be provided to select how literal quotes
                // are to be specified:
                // IE. "\\"" or "\"\""
                qsb.append(ESCAPE_CHAR)
                   .append(q); // \\"
     ...
     *** 
     * @param ch
     *            The quoted char array to parse
     *** @param a
     *            The starting index within the char array to begin parsing
     *** @param sb
    ...
    public static int parseQuote(char ch[],
        ...
        // - Java == 0x00
        // - MySQL == 0x0030
        // 'parseOctal' currently forced to false in order to support MySQL dump
        // files.
        ...
        if ((ch[a] == '\"') || (ch[a] == '\'')) { // quoted string
            ...
            for (; (a < chLen) && (ch[a] != quote); a++) {
                ...
                if (((a + 1) < chLen) && (ch[a] == '\\')) {
                    ...
                    if (parseOctal) {
                        ...
                        if (n > a) {
                            ...
                            try {
                                ...
                                // highly unlikely, since we pre-qualified the
                                // parsed value
                                LogUtil.error("Unable to parse octal: "
                                              + octalStr);
                                // sb.append("?");
                            }
                            a = n - 1; // reset a to last character of octal
                                       // value
                            continue;
                        }
                    }
                    ...
                    if (ch[a] == '0') { // "\0" (this is the only 'octal' value
                                        // that is allowed
                        sb.append((char) 0);
                    } else if (ch[a] == 'r') { // "\r"
                        sb.append('\r'); // ch[a]);
     ...
     *            The object to parse
     *** @param dft
     *            The default Number value if unable to parse the specified
     *            object
     *** @return The parsed Number value
     **/
    public static <T> Number parseNumber(Object data,
                                         Class<?> numberClass,
                                         Number dft) {
        if (data == null) {
            return dft;
        } else if ((numberClass == null)
    ...
    /**
     *** Parse the specified byte array, representing a IEEE 754 floating-point
     * into a double value
     *** 
     * @param b
     *            The byte array to parse
     *** @param ofs
     *            The offset within the byte array to begin parsing
     *** @param isBigEndian
     *            True if the IEEE 754 with the byte array in in BigEndian order
     *** @param dft
    ...
    public static double parseDouble(byte b[],
        ...
        long doubleLong = 0L;
        if (isBigEndian) {
            doubleLong = (((long) b[i + 0] & 0xFF) << (7 * 8))
                         + (((long) b[i + 1] & 0xFF) << (6 * 8))
                         + (((long) b[i + 2] & 0xFF) << (5 * 8))
                         + (((long) b[i + 3] & 0xFF) << (4 * 8))
                         + (((long) b[i + 4] & 0xFF) << (3 * 8))
            ...
            doubleLong = (((long) b[i + 7] & 0xFF) << (7 * 8))
                         + (((long) b[i + 6] & 0xFF) << (6 * 8))
                         + (((long) b[i + 5] & 0xFF) << (5 * 8))
                         + (((long) b[i + 4] & 0xFF) << (4 * 8))
                         + (((long) b[i + 3] & 0xFF) << (3 * 8))
     ...
     *** 
     * @param data
     *            The Object array to parse
     *** @param dft
     *            The default values used if unable to parse a specific entry in
     *            the Object array
     ...
     *            The object to parse
     *** @param dft
     *            The default double value if unable to parse the specified
     *            object
    ...
    public static double parseDouble(String data,
                                     double dft) {
        return StringTools.parseDouble(new FilterNumber(data, Double.class),
                                       dft);
    }
     ...
     *** 
     * @param num
     *            The FilterNumber to parse
     *** @param dft
     *            The default double value if unable to parse the specified
     *            object
     *** @return The parsed double value
     **/
    public static double parseDouble(FilterNumber num,
                                     double dft) {
    ...
    public static float parseFloat(byte b[],
        ...
        int floatInt = 0;
        if (isBigEndian) {
            floatInt = (((int) b[i + 0] & 0xFF) << (3 * 8))
                       + (((int) b[i + 1] & 0xFF) << (2 * 8))
                       + (((int) b[i + 2] & 0xFF) << (1 * 8))
                       + ((int) b[i + 3] & 0xFF);
        } else {
            floatInt = (((int) b[i + 3] & 0xFF) << (3 * 8))
                       + (((int) b[i + 2] & 0xFF) << (2 * 8))
                       + (((int) b[i + 1] & 0xFF) << (1 * 8))
                       + ((int) b[i + 0] & 0xFF);
        }
     ...
     *            The object to parse
     *** @param dft
     *            The default float value if unable to parse the specified
     *            object
     ...
     *** @return The parsed float value
     **/
    public static float parseFloat(FilterNumber num,
                                   float dft) {
     ...
     *** @return The parsed long value
     **/
    public static long parseLong(FilterNumber num,
                                 long dft) {
        if ((num != null) && num.supportsType(Long.class)) {
            if (num.isHex()) {
                ...
                try {
                    ...
                    // Since 'FilterNumber' makes sure that only digits are
                    // parsed,
                    // this likely means that the specified digit string is too
                    // large
                    // for this required data type. Our last ditch effort is to
                    // attempt to convert it to a BigInteger and extract the
                    // lower
     ...
     *            The String to test
     *** @param strict
     *            True to test for a strict long value (ie. does not contain any
     *            other superfluous trailing characters), false to allow for
     *            other non-critical trailing characters.
     *** @return True if the specified String contains a valid long value
     **/
     ...
     *            The default values used if unable to parse a specific entry in
     *            the Object array
     *** @return The parsed int array add by zhbwang 20130507
     **/
    public static Integer[] parseInteger(Object data[],
        ...
        if (data == null) {
            ...
            Integer valList[] = new Integer[data.length];
            for (int i = 0; i < data.length; i++) {
                valList[i] = StringTools.parseInt(data[i], dft);
            }
            return valList;
     ...
     *            The Object array to parse
     *** @param dftList
     *            The int array from which default values are used if unable to
     *            parse a specific entry in the Object array
     *** @return The parsed int array
     **/
    /*
     * public static int[] parseInt(Object data[], int dftList[]) { if
     * (ListTools.isEmpty(data)) { return dftList; } else { int dftLast =
     * ((dftList != null) && (dftList.length > 0))? dftList[dftList.length - 1]
     * : 0; int intList[] = new int[data.length]; for (int i = 0; i <
     * data.length; i++) { int d = ((dftList != null) && (dftList.length > i))?
     * dftList[i] : dftLast; intList[i] = StringTools.parseInt(data[i], d); }
     * return intList; } }
     */
     ...
     *** @return The parsed int value
     **/
    public static int parseInt(FilterNumber num,
                               int dft) {
     ...
     *            The String to test
     *** @param strict
     *            True to test for a strict int value (ie. does not contain any
     *            other superfluous trailing characters), false to allow for
     *            other non-critical trailing characters.
     *** @return True if the specified String contains a valid int value
     **/
     ...
     *** @return True if the specified String contains a valid int value
     **/
    public static boolean isInt(Object data,
                                boolean strict) {
        if (data == null) {
            return false;
        } else if (data instanceof Number) {
     ...
     *            The object to parse
     *** @param dft
     *            The default short value if unable to parse the specified
     *            object
     *** @return The parsed short value
     **/
    public static int parseShort(Object data,
                                 short dft) {
        if (data == null) {
            return dft;
        } else if (data instanceof Number) {
     ...
     *** @return The parsed short value
     **/
    public static short parseShort(FilterNumber num,
                                   short dft) {
        if ((num != null) && num.supportsType(Short.class)) {
            if (num.isHex()) {
                return (short) StringTools.parseHexLong(num.getValueString(),
                                                        dft);
            } else {
                try {
     ...
     *** @return True if the specified String contains a valid short value
     **/
    public static boolean isShort(String data,
                                  boolean strict) {
        if (StringTools.isBlank(data)) {
            return false;
        } else {
     ...
     *            The object to parse
     *** @param dft
     *            The default BigInteger value if unable to parse the specified
     *            object
     *** @return The parsed BigInteger value
     **/
    public static BigInteger parseBigInteger(Object data,
                                             BigInteger dft) {
        if (data == null) {
            return dft;
     ...
     *** @return The parsed BigInteger value
     **/
    public static BigInteger parseBigInteger(String data,
                                             BigInteger dft) {
        return StringTools.parseBigInteger(new FilterNumber(data,
                                                            BigInteger.class),
                                           dft);
    }
     ...
     *** @return The parsed BigInteger value
     **/
    public static BigInteger parseBigInteger(FilterNumber num,
                                             BigInteger dft) {
     ...
     *            The String to test
     *** @param strict
     *            True to test for a strict BigInteger value (ie. does not
     *            contain any other superfluous trailing characters), false to
     *            allow for other non-critical trailing characters.
     *** @return True if the specified String contains a valid BigInteger value
     **/
     ...
     *** @return True if the specified String contains a valid BigInteger value
     **/
    public static boolean isBigInteger(Object data,
                                       boolean strict) {
        if (data == null) {
            return false;
        } else if (data instanceof Number) {
    ...
    public static class FilterNumber {
        ...
        public FilterNumber(String val,
                            Class<?> type) {
            ...
            /* end of digits? */
            String num;
            if ((p >= vlen) || Long.class.isAssignableFrom(type)
        ...
        public <T> boolean supportsType(Class<T> ct) {
            if ((this.numStr != null) && (this.type != null) && (ct != null)) {
                if (this.type.isAssignableFrom(ct)) {
                    ...
                    // (Double/Float/BigInteger/Long/Integer/Short/Byte)
                    return true;
                }
                // types are not the same, check for valid promotions
                if (Short.class.isAssignableFrom(ct)) {
                    // return this.supportsType(Byte.class); // ct is 'Short',
                    // check for promotion from 'Byte'
                    return this.type.isAssignableFrom(Byte.class);
                } else if (Integer.class.isAssignableFrom(ct)) {
                    // return this.supportsType(Short.class); // ct is
                    // 'Integer', check for promotion from 'Short'
                    return this.type.isAssignableFrom(Short.class)
                           || this.type.isAssignableFrom(Byte.class);
                } else if (Long.class.isAssignableFrom(ct)) {
                    // return this.supportsType(Integer.class); // ct is 'Long',
                    // check for promotion from 'Integer'
                    return this.type.isAssignableFrom(Integer.class)
                           || this.type.isAssignableFrom(Short.class)
                           || this.type.isAssignableFrom(Byte.class);
                } else if (BigInteger.class.isAssignableFrom(ct)) {
                    // return this.supportsType(Long.class); // ct is
                    // 'BigInteger', check for promotion from 'Long'
                    return this.type.isAssignableFrom(Long.class)
                           || this.type.isAssignableFrom(Integer.class)
                           || this.type.isAssignableFrom(Short.class)
                           || this.type.isAssignableFrom(Byte.class);
                } else if (Float.class.isAssignableFrom(ct)) {
                    // return this.supportsType(BigInteger.class); // ct is
                    // 'Float', check for promotion from 'BigInteger'
                    return this.type.isAssignableFrom(BigInteger.class)
                           || this.type.isAssignableFrom(Long.class)
                           || this.type.isAssignableFrom(Integer.class)
                           || this.type.isAssignableFrom(Short.class)
                           || this.type.isAssignableFrom(Byte.class);
                } else if (Double.class.isAssignableFrom(ct)) {
                    // return this.supportsType(Float.class); // ct is 'Double',
                    // check for promotion from 'Float'
                    return this.type.isAssignableFrom(Float.class)
            ...
            if ((this.numStr != null) && (this.type != null)) {
                try {
                    if (Byte.class.equals(this.type)) {
                        ...
                        LogUtil.error("Unkrecognized Number type: "
                                      + StringTools.className(this.type));
                        return dft;
                    }
                } catch (NumberFormatException nfe) {
                    // should not occur
                    LogUtil.error("Number conversion error", nfe);
                    return dft;
                }
            }
            return dft;
     ...
     *            The object to parse
     *** @param dft
     *            The default boolean value if unable to parse the specified
     *            object
    ...
    /**
     *** Returns the hex character for the least significant nybble of the
     * specified byte
     *** 
     * @param nybble
     *            The value to convert to a hex character. Only the least
     *            significant nybble of this byte will be used to convert to the
     *            hex character.
    ...
    /**
     *** Parse the specified String, containing a hex representation, into a byte
     * array
     *** 
     * @param data
     *            The String containing the hex character values
     *** @param dft
     *            The default byte array return if unable to convert the
    ...
    public static byte[] parseHex(String data,
        ...
        if (data != null) {
            ...
            // scan until we find an invalid character (or the end of the
            // string)
            for (int i = 0; i < s.length(); i++) {
     ...
     *** 
     * @param data
     *            The String hex representation to convert to a String
     *** @param dft
     *            The default int value to return if unable to convert the
     *            specified String hex representation.
     *** @return The parse int value
     **/
    public static int parseHex(String data,
                               int dft) {
     ...
     *** @return The parse int value
     **/
    public static int parseHexInt(String data,
                                  int dft) {
        return (int) StringTools.parseHexLong(data, (long) dft);
    }
     ...
     *            The String hex representation to convert to a String
     *** @param dft
     *            The default long value to return if unable to convert the
     *            specified String hex representation.
     *** @return The parse long value
     **/
    public static long parseHex(String data,
                                long dft) {
     ...
     *** @return The parse long value
     **/
    public static long parseHexLong(String data,
                                    long dft) {
     ...
     *** 
     * @param data
     *            The String representation of the hex characters to test
     *** @param strict
     *            True to check for strict hex character values, false to allow
     *            for trailing superfluous characters.
     *** @return True if the specified String contains a valie hex representation,
     *         false otherwise.
     **/
    ...
    /**
     *** This method prints the specified byte array to a String hex
     * representation showing the contained bytes with corresponding displayed
     ...
     *            The byte array to convert to a String representation
     *** @param bOfs
     *            The starting index where the byte array contents will be
     *            dipsplayed
     *** @param bLen
     *            The number of byte to display from the specified byte array
     *** @param blockLen
     *            The number of bytes display on a single row
     *** @param showAscii
     *            True to display the
     ...
     *** @param sb
     *            The destination StringBuffer where the hex String is placed.
     *            If null, a new StringBuffer will be created.
    ...
    public static String toHexString(byte b) {
        return StringTools.toHexString(b, null)
                          .toString();
    }
     ...
     *** @param ofs
     *            The offset into the byte array to start the hex conversion
     *** @param len
     ...
     **/
    public static String toHexString(byte b[]) {
        return StringTools.toHexString(b, 0, -1, null)
                          .toString();
    }
    ...
    public static String toHexString(byte b[],
                                     int ofs,
                                     int len) {
        return StringTools.toHexString(b, ofs, len, null)
                          .toString();
    }
     ...
     *** 
     * @param val
     *            The BigInteger value to convert to hex
     *** @param bitLen
     *            The length of significant bits to include in the hex
     *            representation
     *** @return The String containing the hex representation
     **/
    public static String toHexString(BigInteger val,
        ...
        if (bitCount > bitLen) {
            val = val.and(BigInteger.ONE.shiftLeft(bitLen)
                                        .subtract(BigInteger.ONE));
        }

        /* hex */
        int nybbleLen = ((bitLen + 7) / 8) * 2;
        StringBuffer hex = new StringBuffer(val.toString(16)
                                               .toUpperCase());
        if (nybbleLen > hex.length()) {
            hex.insert(0,
                       StringTools.replicateString("0",
                                                   nybbleLen - hex.length()));
        }
        return hex.toString();
    ...
    public static String toHexString(long val,
        ...
        int nybbleLen = ((bitLen + 7) / 8) * 2;
        StringBuffer hex = new StringBuffer(Long.toHexString(val)
                                                .toUpperCase());
        // LogUtil.info("NybbleLen: " + nybbleLen + " : " + hex + " [" +
        // hex.length());
     ...
     *** @param ofs
     *            The offset into the byte array to start the binary conversion
     *** @param len
     *            The number of bytes to convert to binary
     *** @param sb
     *            The destination StringBuffer where the binary String is
     *            placed. If null, a new StringBuffer will be created.
     *** @return The StringBuffer where the String binary representation is placed
     **/
    public static StringBuffer toBinaryString(byte b[],
                                              int ofs,
                                              int len,
                                              StringBuffer sb) {
        if (sb == null) {
    ...
    public static String toBinaryString(byte b[]) {
        return StringTools.toBinaryString(b, new StringBuffer())
                          .toString();
    }
    ...
    public static String toBinaryString(byte b[],
                                        int ofs,
                                        int len) {
        return StringTools.toBinaryString(b, ofs, len, null)
                          .toString();
    }
     ...
     **/
    public static String toBinaryString(byte b) {
        return StringTools.toBinaryString(b, null)
                          .toString();
    }
     ...
     *** @return The StringBuffer where the String binary representation is placed
     **/
    public static StringBuffer toBinaryString(int i,
                                              StringBuffer sb) {
    ...
    public static String toBinaryString(int i) {
        return StringTools.toBinaryString(i, null)
                          .toString();
    }
     ...
     *** @param bc
     *            The bit-count to return
     *** @param sb
     *            The destination StringBuffer where the binary String is
     *            placed. If null, a new StringBuffer will be created.
     *** @return The StringBuffer where the String binary representation is placed
     **/
    public static StringBuffer toBinaryString(long i,
                                              int bc,
                                              StringBuffer sb) {
        if (bc <= 0) {
            bc = 64;
    ...
    public static String toBinaryString(long i) {
        return StringTools.toBinaryString(i, -1, null)
                          .toString();
    }
    ...
    /**
     *** Sets the first character of the beginning of each new word in the
     * specified String to upper-case, and sets the remaining characters in each
     * word to lower-case.
     *** 
     * @param s
     ...
     *** @param m
     *            The pattern String
     *** @return True if the test String starts with the pattern String, false
     *         otherwise.
     **/
     ...
     *         otherwise.
     **/
    public static boolean startsWith(String t,
                                     String p) {
        if ((t != null) && (p != null)) {
    ...
    /**
     *** Returns true if the specified text String starts with the pattern String
     * without regard to case (a method that should be on the String class
     * itself, but isn't)
     *** 
     * @param t
     *            The test String
     ...
     *         otherwise.
     **/
    public static boolean startsWithIgnoreCase(String t,
                                               String p) {
        if ((t != null) && (p != null)) {
     ...
     *** @param p
     *            The pattern String
     *** @return True if the test String ends with the pattern String, false
     *         otherwise.
     **/
    public static boolean endsWith(String t,
                                   String p) {
        if ((t != null) && (p != null)) {
            return t.endsWith(p);
        } else {
     ...
     *         otherwise.
     **/
    public static boolean endsWithIgnoreCase(String t,
                                             String p) {
        if ((t != null) && (p != null)) {
            return t.toLowerCase()
    ...
    /**
     *** Returns true if the specified text String ends with one of the pattern
     * Strings in the specified array, without regard to case.
     *** 
     * @param t
     *            The test String
     *** @param p
     *            An array of pattern Strings
     *** @return True if the test String ends with any pattern String, false
     *         otherwise.
     **/
    public static boolean endsWithIgnoreCase(String t,
                                             String p[]) {
    ...
    /**
     *** Returns index/position of the second pattern String within the first test
     * String
     *** 
     * @param t
     *            The test String
     *** @param m
     *            The pattern String
     *** @return The position of the pattern String within the test String, or -1
     *         if the pattern String does not exist within the test String.
     **/
    public static int indexOfIgnoreCase(String t,
                                        String m) {
        if ((t != null) && (m != null)) {
            return t.toLowerCase()
     ...
     *** @return True if the pattern string is contained within the target string
     **/
    public static boolean containsIgnoreCase(String t,
                                             String m) {
        if ((t != null) && (m != null)) {
            return (t.toLowerCase()
    ...
    /**
     *** Returns the position of the specified pattern character within the
     * character array
     *** 
     * @param A
     *            The character array
     ...
     *** @return The position of the pattern character within the test character
     *         array, or -1 if the pattern character does not exist within the
     ...
     *** @param b
     *            The pattern byte
     *** @return The position of the pattern byte within the test byte array, or
     *         -1 if the pattern byte does not exist within the test byte array
     **/
    ...
    /**
     *** Parses the specified "," delimited String to an array of Strings. Quoted
     * values are allowed within the delimited String and will be parsed as
     * literal values in the String array.
     *** 
     * @param s
     ...
     * Quoted values are allowed within the delimited String and will be parsed
     * as literal values in the String array.<br>
     *** NOTE: if the last entry is non-quoted and blank, it will be ignored.
     *** 
     * @param s
     *            The character delimited String to parse
     *** @param arrayDelim
    ...
    public static String[] parseArray(String s,
        ...
        for (int a = 0;;) {
            if (a >= len) {
                // we're here because we've parse a delimiter, but haven't
                // gotten an entry
                ...
                a++; // skip delimiter
            } else if ((ch[a] == ARRAY_DOUBLE_QUOTE)
                       || (ch[a] == ARRAY_SINGLE_QUOTE)) { // token = '\"'
                // parse quoted string
                StringBuffer sb = new StringBuffer();
    ...
    /**
     *** Encodes an array of Strings/Objects into a single String, using the
     * specified character as the String field delimiter.
     ***
     * @param list
     *            The array of Strings/Objects to encode
     *** @param delim
     *            The character delimter
     *** @param alwaysQuote
     *            True to always quote each String field. If false, a String
     *            field will only be quoted if it contains embedded spaces or
     *            other characters that need to be specified literally.\
     ...
     *            The array of Strings/Objects to encode
     *** @param ofs
     *            The offset within list to begin encoding into the returned
     *            String
     *** @param max
     *            The number of String fields to include from the specified list
     *** @param delim
     *            The character delimter
     *** @param alwaysQuote
     *            True to always quote each String field. If false, a String
    ...
    /**
     *** Encodes an array/list of Strings/Objects into a single String, using the
     * specified character as the String field delimiter.
     ***
     * @param list
     *            The Object containing an array or list of Strings to encode
     *** @param delim
     *            The character delimter
     *** @param alwaysQuote
     *            True to always quote each String field. If false, a String
    ...
    public static String encodeArray(Object list,
        ...
        if (list == null) {
            ...
            // list
            return encodeArray(ListTools.toArray((List) list),
                                           ofs,
                                           max,
                                           delim,
                                           alwaysQuote);
            ...
            return StringTools.encodeArray((Object[]) list,
                                           ofs,
                                           max,
                                           delim,
                                           alwaysQuote);
        } else if (list.getClass()
                       .isArray()) {
    ...
    /**
     *** Encodes a list of Strings/Objects into a single String, using the
     * specified character as the String field delimiter.
     ***
     * @param list
    ...
    public static String encodeArray(List<Object> list,
                                     char delim,
                                     boolean alwaysQuote) {
        return encodeArray(ListTools.toArray(list),
                                       delim,
                                       alwaysQuote);
    }
    ...
    /**
     *** Encodes a list of Strings/Objects into a single String, using the ","
     * character as the String field delimiter. All returned String fields will
     * be quoted.
     ***
     * @param list
     *            The List containing the Strings to encode
    ...
    /**
     *** Encodes a list of Strings/Objects into a single String, using the ","
     * character as the String field delimiter.
     ***
     * @param list
     *            The List containing the Strings to encode
     *** @param alwaysQuote
    ...
    /**
     *** Encodes an Object containing a list of Strings into a single String,
     * using the "," character as the String field delimiter. All returned
     * String fields will be quoted.
     ***
     * @param list
     *            The array containing the Strings to encode
     *** @return The encoded String
    ...
    /**
     *** Encodes an Object containing a list of Strings into a single String,
     * using the "," character as the String field delimiter.
     ***
     * @param list
     *            The array containing the Strings to encode
     *** @param alwaysQuote
    ...
    /**
     *** Parse the character delimited input String into an array of Strings
     * (similar to Perl's "split" function).<br>
     *** Quoted Strings are parsed literally.<br>
     *** If the input String is non-null, this method returns an array with
     * at-least one entry. If the input String is null, an empty (non-null)
     * String array is returned.<br>
     ***
     * @param value
     *            The String to parse
     *** @param delim
     ...
     *            The String to parse
     *** @param sdelim
     *            The character delimiters (all characters in this String are
     *            considered individual delimiter candidates)
     *** @return The array of parse Strings
     **/
    public static String[] parseString(String value,
                                       String sdelim) {
     ...
     *** @return The array of parse Strings
     **/
    public static String[] parseString(StringBuffer value,
                                       char delim) {
        if (value == null) {
            return new String[0];
        } else {
            return StringTools.parseString(value.toString(),
     ...
     **/
    public static String[] split(StringBuffer value,
                                 char delim) {
        return StringTools.parseString(value, String.valueOf(delim), true);
    }
    ...
    /**
     *** Concatenates the specified String array into a single String using the
     * specified character as the delimiter. Null elements in the input String
     * array are skipped.
     ***
     * @param val
    ...
    /**
     *** Concatenates the specified String array into a single String using the
     * specified String as the delimiter. Null elements in the input String
     * array are skipped.
     ***
     * @param val
     *            The input String array
    ...
    /**
     *** Concatenates the specified List objects into a single String using the
     * specified String as the delimiter. Null elements in the input list are
     * skipped.
     ***
     * @param list
    ...
    /**
     *** Concatenates the specified int array into a single String using the
     * specified String as the delimiter.
     ***
     * @param val
     *            The input int array
     *** @param delim
     ...
     *** @return The concatinated String
     **/
    public static String join(int val[],
                              char delim) {
        StringBuffer sb = new StringBuffer();
        if (val != null) {
            for (int i = 0; i < val.length; i++) {
    ...
    /**
     *** Concatenates the specified long array into a single String using the
     * specified String as the delimiter.
     ***
     * @param val
     ...
     *** @return The concatinated String
     **/
    public static String join(long val[],
                              char delim) {
        StringBuffer sb = new StringBuffer();
        if (val != null) {
            for (int i = 0; i < val.length; i++) {
    ...
    /**
     *** Concatenates the specified double array into a single String using the
     * specified String as the delimiter.
     ***
     * @param val
     ...
     *** @return The concatinated String
     **/
    public static String join(double val[],
                              char delim) {
        StringBuffer sb = new StringBuffer();
        if (val != null) {
            for (int i = 0; i < val.length; i++) {
    ...
    private static boolean _isSepChar(char ch,
                                      char sepChar) {
    ...
    private static int _indexOfChar(String S,
                                    char C[]) {
        int slen = S.length();
    ...
    /**
     *** Parses a series of properties in the specified String into a property
     * map. Properties are specified as "key=value" pairs, and are separated
     * from other property specifications by whitespace. Neither the property
     * keys, or values may contain whitespace.
     ***
     * @param props
    ...
    public static Map<String, String> parseProperties(String props) {
        return StringTools.parseProperties(props,
                                           StringTools.PropertySeparatorChar,
                                           StringTools.KeyValSeparatorChars,
     ...
     *** Parses a series of properties in the specified String into a property
     * map. Properties are specified as "key=value" pairs, and are separated
     * from other property specifications by the specified property separator
     * character. Neither the property keys, or values may contain the specified
     ...
     *            The String containing the list of properties
     *** @param properties
     *            The map where the parsed properties will be placed. If null, a
     *            mew map object will be created.
     *** @return The map containing the parsed property list
     **/
    public static Map<String, String> parseProperties(String props,
     ...
     *            The property separator charactor (ie. ' ', or ';')
     *** @param keyValSep
     *            The key/value separator chars (ie. '=' or ':')
     *** @param properties
     *            The map where the parsed properties will be placed. If null, a
     *            mew map object will be created.
     *** @return The map containing the parsed property list
    ...
    public static Map<String, String> parseProperties(String props,
        ...
        for (; a < argsLen;) {
            ...
            while ((a < argsLen)
                   && (Character.isWhitespace(args[a]) || _isSepChar(args[a],
                                                                     propSep))) {
                a++;
            }

            /* prop name */
            // scan until first Whitespace, PropertySeparator, or
            // KeyValSeparator
            StringBuffer propName = new StringBuffer();
            for (; (a < argsLen) && !Character.isWhitespace(args[a])
                   && !_isSepChar(args[a], propSep)
                   && !_isSepChar(args[a], keyValSep); a++) {
                propName.append(args[a]);
            }

            /* skip whitespace? */
            if (!spacePropSep) {
                // embedded spaced allowed if the property separator is not a
                // space
            ...
            if ((a < argsLen) && _isSepChar(args[a], keyValSep)) {
                a++; // skip past '='
                if (!spacePropSep) {
                    // skip whitespace (embedded spaced allowed if the property
     ...
     ***
     * @param src
     *            The input String from which the characters will be removed
     *** @param chars
     *            The list of characters which will be removed from the input
     *            String
     *** @return The stripped String
     **/
    public static String stripChars(String src,
                                    char chars[]) {
        StringBuffer sb = new StringBuffer();
     ...
     *            The input String from which the characters will be removed
     *** @param chars
     *            The String of characters which will be removed from the input
     *            String
     *** @return The stripped String
     **/
    public static String stripChars(String src,
     ...
     *            The character which will be removed from the input String
     *** @param stripType
     *            May be either STRIP_INCLUDE to include only the specified
     *            character, or STRIP_EXCLUDE to exclude the specified
     *            character.
     *** @return The stripped String
     **/
    public static String stripChars(String src,
     ...
     *** @param stripType
     *            May be either STRIP_INCLUDE to include only the specified
     *            characters, or STRIP_EXCLUDE to exclude the specified
     *            characters.
     *** @return The stripped String
     **/
    public static String stripChars(String src,
    ...
    /**
     *** Replaces specific characters in the input String with the specified
     ...
     **/
    public static String replaceChars(String src,
                                      char ch,
                                      char repChar) {
        return StringTools.replaceChars(src,
                                        String.valueOf(ch),
                                        String.valueOf(repChar));
    }
     ...
     *            The input String
     *** @param chars
     *            The characters to replace (any character found in this String
     *            will be replaced)
     *** @param repChar
     *            The replacement character
     *** @return The Stirng containing the replaced characters
    ...
    /**
     *** Replaces all whitespace found in the input String with the specified
     * replacement character
     ***
     * @param src
     *            The input String
    ...
    /**
     *** ValueFilter interface. Used to provide key/value replacement value
     * filtering
     **/
    ...
    /**
     *** Replaces all occurances of "${key}" with the value returned by the
     * KeyValueMap interface 'getKeyValue' method.
     ***
     * @param text
     *            The text containing the "${key}" fields
     *** @param keyMap
     *            The KeyValueMap object used to retrieve values for the
     *            specific 'keys'
     **/
    ...
    public static String replaceKeys(String text,
        ...
        for (; (ks >= 0);) {
            ...
            if (d >= 0) {
                dftStr = keyArgDft.substring(d + dftDelim.length()); // leave
                                                                     // default
                                                                     // as-is
                                                                     // (untrimmed)
                keyArg = keyArgDft.substring(0, d)
                                  .trim();  // trim key
                // LogUtil.info("Found Default: " + keyArg + " ==> " + dftStr);
            } else {
                dftStr = "";
                keyArg = keyArgDft;
            }
    ...
    /**
     *** Returns a String containing the 'pattern' String replicated 'count'
     * times.
     ***
     * @param pattern
     *            The pattern String
     ...
     **/
    public static String replicateString(String pattern,
                                         int count) {
    ...
    /**
     *** Pads the input String on the right with specified number of pad
     * characters. If the length of the String is equal-to, or greater-than, the
     * specified length, the the input String is returned as-is.
     ***
     * @param s
     *            The input String
     *** @param padChar
    ...
    public static String padRight(String s,
        ...
        if (s == null) {
            ...
            return s
                   + StringTools.replicateString(String.valueOf(padChar),
                                                 len - s.length());
        }
    }

    /**
     *** Pads the input String on the right with specified number of ' '
     * characters. If the length of the String is equal-to, or greater-than, the
     * specified length, the the input String is returned as-is.
     ***
     * @param s
    ...
    /**
     *** Pads the input String on the left with specified number of pad
     * characters. If the length of the String is equal-to, or greater-than, the
     * specified length, the the input String is returned as-is.
     ***
     * @param s
    ...
    public static String padLeft(String s,
        ...
        if (s == null) {
            ...
            return StringTools.replicateString(String.valueOf(padChar),
                                               len - s.length())
                   + s;
        }
    }
    ...
    /**
     *** Pads the input String on the left with specified number of ' '
     * characters. If the length of the String is equal-to, or greater-than, the
     * specified length, the the input String is returned as-is.
     ***
     * @param s
    ...
    public static String encodeNewline(String text) {
        if (text != null) {
            return StringTools.encodeNewline(new StringBuffer(text))
                              .toString();
        } else {
            return null;
        }
    ...
    public static String decodeNewline(String text) {
        if (text != null) {
            return StringTools.decodeNewline(new StringBuffer(text))
                              .toString();
        } else {
            return null;
        }
    ...
    public static String replace(String text,
                                 ...
                                 String val) {
        if (text != null) {
            return StringTools.replace(new StringBuffer(text), key, val)
                              .toString();
        } else {
            return null;
        }
     ...
     **/
    public static StringBuffer replace(StringBuffer sb,
                                       String key,
                                       String val) {
        if (sb != null) {
            int s = 0;
    ...
    /**
     *** Within the input 'target' String, replaces all occurances of the regular
     * expression 'regex', with the 'val' String.
     ***
     * @param target
     *            The input String
     *** @param regex
     ...
     **/
    public static String regexReplace(String target,
                                      String regex,
                                      String val) {
    ...
    /**
     *** Within the input 'target' StringBuffer, replaces all occurances of the
     * regular expression 'regex', with the 'val' String.
     ***
     * @param target
     *            The input StringBuffer
     *** @param regexKey
     ...
     **/
    public static StringBuffer regexReplace(StringBuffer target,
                                            String regexKey,
                                            String val) {
     ...
     *** @return True if the target String matches the specified regular
     *         expression
     **/
    public static boolean regexMatches(String target,
                                       String regex) {
        // LogUtil.info("Regex=" + regex + ", Target=" + target);
    ...
    /**
     *** Returns regular expressing index of the specified regular expression
     ...
     *** @param regex
     *            The regular expression
     *** @return The RegexIndex of the location of the regular expression within
     *         the target String
     **/
    public static RegexIndex regexIndexOf(String target,
                                          String regex) {
     ...
     *            The regular expression
     *** @param ndx
     *            The position within the target String to start searching for
     *            the regular expression
     *** @return The RegexIndex of the location of the regular expression within
     *         the target String
     **/
    ...
    /**
     *** ReplacementMap interface (see KeyValueMap interface)
     **/
    ...
    /**
     *** Searches the input String for key variables (determined by the specified
     * start/end delimiters) and replaces matching keys with the values
     * specified in the replacement array
     ***
     * @param text
     *            The target String
     *** @param startDelim
     *            The pattern used to determine the start of a 'key' variable
     *** @param endDelim
     ...
     *            An array containing key/value pairs
     *** @param htmlFilter
     *            True to encode the resulting key value for display within an
     *            html context
     *** @return The String containing the replaced key variables
     **/
    public static String insertKeyValues(String text,
     ...
     *** Searches the input String for key variables (determined by the specified
     * start/end delimiters) and replaces matching keys with the values
     * specified in the replacement map
     ***
     * @param text
     *            The target String
     *** @param startDelim
     ...
     *            The pattern used to determine the end of a key variable
     *** @param rmap
     *            A StringTools.KeyValueMap (previously ReplacementMap)
    ...
    public static String insertKeyValues(String text,
                                         ...
                                         boolean htmlFilter) // ReplacementMap
    {
        String argDelim = ARG_DELIM;
        // Notes:
        // - This method looks for patterns similar to "... ${key} ..."
        // - Key arguments and value defaults are not considered (however this
        // should be)
        // - If the pattern "${key:arg=default}" is encountered, the entire
        // "key:arg=default" is considered the key
        ...
        while (s < sb.length()) {
            ...
            // no longer necessary, since '_findEndDelimiter' returns the proper
            // ending-delimiter pointer
            /*
             * int sn = sb.indexOf(startDelim, s + startDelimLen); // next start
             * delimiter if ((sn >= 0) && (e > sn)) { // ending delimiter is
             * beyond next start delimiter // likely a syntax error, skip the
             * current var replacement and move to the next s = sn; // reset
             * starting delimiter continue; }
             */

            /* extract "key:arg=default" */
            String keyArgDft = sb.substring(s + startDelimLen, e)
                                 .trim(); // trim key
            ...
            String val = (rmap != null) ? rmap.getKeyValue(key, arg, dftStr) : ("?"
                                                                                + key + "?");
            if (val != null) {
                // LogUtil.info("Found Key: " + key + " [replace with '" + val +
                // "']");
                sb.replace(s,
                           e + endDelimLen,
                           (htmlFilter ? StringTools.htmlFilterText(val) : val));
                s += val.length();
            } else {
                // LogUtil.info("Key not found: " + key + " [replace with '']");
                // "${key}" is left as-is
                s = e + endDelimLen;
            }
    ...
    private static int _findEndDelimiter(StringBuffer sb,
                                         int c,
                                         char sd[],
                                         char ed[]) {
        ...
        for (; c < sblen;) {
            ...
            if (sb.charAt(c) == sd[0]) {
                ...
                for (; (x < sd.length) && ((c + x) < sblen)
                       && (sb.charAt(c + x) == sd[x]); x++)
                    ;
                if (x == sd.length) {
                    level++;
                    c += x;
            ...
            if (sb.charAt(c) == ed[0]) {
                int x = 1; // index '0' already matched
                for (; (x < ed.length) && ((c + x) < sblen)
                       && (sb.charAt(c + x) == ed[x]); x++)
                    ;
                if (x == ed.length) {
                    level--;
                    if (level == 0) {
     ...
     *** @param b2
     *            Second byte array
     *** @return 0 if byte arrays are equal, < 0 if the first non-matching byte is
     *         less than the second, > 0 if the first non-matching byte is
     ...
     *** @param s
     *            Second String value
     *** @return 0 if the byte array is equal to the specified String, < 0 if the
     *         first non-matching byte is less than the correspoinding
     *         character/byte in the String, > 0 if the first non-matching byte
     *         is greater than the correspoinding character/byte in the String
     **/
    public static int compare(byte b1[],
                              String s) {
        return StringTools.compare(b1,
                                   ((s != null) ? StringTools.getBytes(s) : null),
                                   -1);
    }
    ...
    public static boolean compareEquals(byte b[],
                                        String s) {
        return StringTools.compareEquals(b,
                                         ((s != null) ? StringTools.getBytes(s) : null),
                                         -1);
    }
    ...
    /**
     *** Compares 2 Strings, returning the index of the character where they
     * differ
     *** 
     * @param s1
     *            First String
     *** @param s2
     *            Second String
     *** @return The index/location where the Strings differ, or -1 if they are
     *         the same
     **/
     ...
     *** @param b2
     *            Second byte array
     *** @return The index/location where the byte arrays differ, or -1 if they
     *         are the same
     **/
    ...
    /**
     *** Format/Convert the specified double value to a String, based on the
    ...
    /**
     *** Format/Convert the specified double value to a String, based on the
     * specified format pattern<br>
     *** If the format String begins with '%', then String.format(...) will be
     * used.<br>
    ...
    public static String format(double val,
        ...
        if (fmt == null) {
            ...
            try {
                ...
                LogUtil.error("Format exception [" + fmt + "]", th);
                s = String.valueOf(val);
            }
        } else {
            s = StringTools._getFormatter(fmt)
                           .format(val);
        }
    ...
    /**
     *** Format/Convert the specified BigInteger value to a String, based on the
     * specified format pattern
     *** 
     * @param val
     *            The BigInteger value to format
     *** @param fmt
     *            The format pattern
     *** @return The String containing the formatted long value
     **/
    public static String format(BigInteger val,
                                String fmt) {
        return format(val, fmt, -1);
    }
    ...
    /**
     *** Format/Convert the specified long value to a String, based on the
     * specified format pattern
     *** 
     * @param val
    ...
    /**
     *** Format/Convert the specified int value to a String, based on the
     * specified format pattern
     *** 
     * @param val
     *            The int value to format
     *** @param fmt
     *            The format pattern
    ...
    /**
     *** Format/Convert the specified short value to a String, based on the
     * specified format pattern
     *** 
     * @param val
     *            The short value to format
     *** @param fmt
     *            The format pattern
     ...
     *** 
     * @param elapsedSec
     *            The elapsed seconds to format
     *** @param fmt
     *            Once of the following constant values:<br>
     ...
     **/
    public static String formatElapsedSeconds(long elapsedSec,
                                              int fmt) {
        StringBuffer sb = new StringBuffer();
        switch (fmt) {
     ...
     *** @return The class name of the specified object. If the specified object
     *         is a Class object, then 'getName()' is used on the object to
     *         return the class name directly.
     **/
    ...
    /**
     *** Compress/Obfuscate the specified long value into a String using the
     * specified alphabet (Note: In this context "compress" means the length of
     * the String representation, and not the number of byte required to
     * represent the long value).
     *** 
     * @param num
     *            The long value to obfuscate
     *** @param alpha
     ...
     *** 
     * @param str
     *            The String from which the long value will be
     *            decompressed/unobfuscated
    ...
    /**
     *** Decompress/Unobfuscate the specified String into a long value using the
     * specified alphabet (this must be the same alphabet used to encode the
     * long value)
     *** 
     * @param str
     *            The String from which the long value will be
     *            decompressed/unobfuscated
     *** @param alpha
     *            The alphabet used to decompress/unobfuscate the long value
     *            (this must be same alphabet used to compress/obfuscate the
     *            long value)
     *** @return The decompressed/unobfuscated long value
     **/
    public static long decompressDigits(String str,
    ...
    public static final String HTML_APOS = "&#39;"; // "&apos;"; <-- doesn't
                                                    // work on IE
    ...
    public static byte[] copyByteArray(byte[] bytes,
        ...
        int min_size = Math.min(length, left);
        // 拷贝。
        byte[] buf = new byte[min_size];
        System.arraycopy(bytes, offset, buf, 0, min_size);
    ...
    public static void main(String argv[]) {
        ...
        LogUtil.info("Current Character set: "
                     + StringTools.getCharacterEncoding());

        /* display current CharacterSet */
        // if (RTConfig.hasProperty("charset")) {
        // LogUtil.info("Character Set: %s",
        // StringTools.getCharacterEncoding());
        // System.exit(0);
        // }

        /* hex test */
        String hex = props.getString(ARG_HEX, null);
        if (hex != null) {
            ...
            if (b != null) {
                ...
                LogUtil.info("String(ASCII): " + StringTools.toStringValue(sb));
                ...
                try {
                    ...
                    LogUtil.info("String(UTF8) : " + utf8SB);
                ...
                LogUtil.info("ERROR: Invalid hex value");
            }
            System.exit(0);
        }

        /* convert to hex */
        String tohex = props.getString("tohex", null);
        if (tohex != null) {
            if (StringTools.isLong(tohex, true)) {
                long val = StringTools.parseLong(tohex, 0L);
                LogUtil.info("Long: (" + val + ") 0x"
                             + StringTools.toHexString(val));
            } else {
                byte b[] = StringTools.getBytes(tohex);
                LogUtil.info("Byte: 0x" + StringTools.toHexString(b));
            }
            System.exit(0);
        }

        /* convert to unicode */
        String tounicode = props.getString("tounicode", null);
        if (tounicode != null) {
            String uc = StringTools.escapeUnicode(tounicode);
            LogUtil.info("String : " + tounicode);
            LogUtil.info("Unicode: " + uc);
            System.exit(0);
        }

        /* convert from unicode */
        String frunicode = props.getString("fromunicode", null);
        if (frunicode != null) {
            String st = StringTools.unescapeUnicode(frunicode);
            LogUtil.info("Unicode: " + frunicode);
            LogUtil.info("String : " + st);
            System.exit(0);
        }

        /* array test */
        String arryStr = props.getString("array", null);
        if (arryStr != null) {
        ...
        /* split test */
        String splitStr = props.getString("split", null);
        if (splitStr != null) {
        ...
        /* replace test */
        String replaceStr = props.getString("replace", null);
        if (replaceStr != null) {
            String m = "1234567890";

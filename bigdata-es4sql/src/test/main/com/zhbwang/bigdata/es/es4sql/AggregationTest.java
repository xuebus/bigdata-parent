...
//
    ...
    public void extendedStatsTest() throws IOException, SqlParseException, SQLFeatureNotSupportedException {
        Aggregations result = query(String.format("SELECT EXTENDED_STATS(age) FROM %s/account", TestsConstants.TEST_INDEX));
    ...
    public void percentileTest() throws IOException, SqlParseException, SQLFeatureNotSupportedException {
        Aggregations result = query(String.format("SELECT PERCENTILES(age) FROM %s/account", TestsConstants.TEST_INDEX));
    ...
    public void percentileTestSpecific() throws IOException, SqlParseException, SQLFeatureNotSupportedException {
        Aggregations result = query(String.format("SELECT PERCENTILES(age,25.0,75.0) x FROM %s/account", TestsConstants.TEST_INDEX));
    ...
    public void multipleGroupBysWithSize() throws Exception {
        ...
        Aggregations result = query(String.format("SELECT COUNT(*) FROM %s/account GROUP BY gender, terms('alias'='ageAgg','field'='age','size'=3)", TestsConstants.TEST_INDEX));
        Terms gender = result.get("gender");
    ...
    public void termsWithSize() throws Exception {
        ...
        Aggregations result = query(String.format("SELECT COUNT(*) FROM %s/account GROUP BY terms('alias'='ageAgg','field'='age','size'=3)", TestsConstants.TEST_INDEX));
    ...
    public void termsWithMissing() throws Exception {
        Aggregations result = query(String.format("SELECT count(*) FROM %s/gotCharacters GROUP BY terms('alias'='name','field'='name.ofHisName','missing'='-999')", TestsConstants.TEST_INDEX));
    ...
    public void termsWithOrder() throws Exception {
        Aggregations result = query(String.format("SELECT count(*) FROM %s/dog GROUP BY terms('field'='dog_name', 'alias'='dog_name', order='desc')", TestsConstants.TEST_INDEX));
        ...
        result = query(String.format("SELECT count(*) FROM %s/dog GROUP BY terms('field'='dog_name', 'alias'='dog_name', order='asc')", TestsConstants.TEST_INDEX));
	...
	public void topHitTest() throws IOException, SqlParseException, SQLFeatureNotSupportedException {
        Aggregations result = query(String.format("select topHits('size'=3,age='desc') from %s/accounts group by gender ", TestsConstants.TEST_INDEX));
		System.out.println(result);
	}
    ...
    public void topHitTest_WithInclude() throws IOException, SqlParseException, SQLFeatureNotSupportedException {
        Aggregations result = query(String.format("select topHits('size'=3,age='desc',include=age) from %s/account group by gender ", TestsConstants.TEST_INDEX));
    ...
    public void topHitTest_WithIncludeTwoFields() throws IOException, SqlParseException, SQLFeatureNotSupportedException {
        Aggregations result = query(String.format("select topHits('size'=3,'include'='age,firstname',age='desc') from %s/account group by gender ", TestsConstants.TEST_INDEX));
        List<Terms.Bucket> buckets = ((Terms) (result.asList().get(0))).getBuckets();
        for (Terms.Bucket bucket : buckets){
            InternalSearchHits hits = (InternalSearchHits) ((InternalTopHits) bucket.getAggregations().asList().get(0)).getHits();
            for(SearchHit hit: hits ){
    ...
    public void topHitTest_WithExclude() throws IOException, SqlParseException, SQLFeatureNotSupportedException {
        Aggregations result = query(String.format("select topHits('size'=3,'exclude'='lastname',age='desc') from %s/account group by gender ", TestsConstants.TEST_INDEX));
        List<Terms.Bucket> buckets = ((Terms) (result.asList().get(0))).getBuckets();
        for (Terms.Bucket bucket : buckets){
            InternalSearchHits hits = (InternalSearchHits) ((InternalTopHits) bucket.getAggregations().asList().get(0)).getHits();
            for(SearchHit hit: hits ){
    ...
    public void topHitTest_WithIncludeAndExclude() throws IOException, SqlParseException, SQLFeatureNotSupportedException {
        Aggregations result = query(String.format("select topHits('size'=3,'exclude'='lastname','include'='firstname,lastname',age='desc') from %s/account group by gender ", TestsConstants.TEST_INDEX));
        List<Terms.Bucket> buckets = ((Terms) (result.asList().get(0))).getBuckets();
        for (Terms.Bucket bucket : buckets) {
            InternalSearchHits hits = (InternalSearchHits) ((InternalTopHits) bucket.getAggregations().asList().get(0)).getHits();
            for (SearchHit hit : hits) {
    ...
    public void testFromSizeWithAggregations() throws Exception {
        final String query1 = String.format("SELECT /*! DOCS_WITH_AGGREGATION(0,1) */" +
                " account_number FROM %s/account GROUP BY gender", TestsConstants.TEST_INDEX);
	...
	public void testSubAggregations() throws  Exception {
		...
		final String query = String.format("SELECT /*! DOCS_WITH_AGGREGATION(10) */" +
                " * FROM %s/account GROUP BY (gender, age), (state) LIMIT 0,10", TestsConstants.TEST_INDEX);
	...
	public void testSimpleSubAggregations() throws  Exception {
		final String query = String.format("SELECT /*! DOCS_WITH_AGGREGATION(10) */ * FROM %s/account GROUP BY (gender), (state) ", TestsConstants.TEST_INDEX);
    ...
    public void geoBounds() throws SQLFeatureNotSupportedException, SqlParseException {
        Aggregations result = query(String.format("SELECT * FROM %s/location GROUP BY geo_bounds(field='center',alias='bounds') ", TestsConstants.TEST_INDEX));
    ...
    public void groupByOnNestedFieldTest() throws Exception {
        Aggregations result = query(String.format("SELECT COUNT(*) FROM %s/nestedType GROUP BY nested(message.info)", TestsConstants.TEST_INDEX));
    ...
    public void groupByTestWithFilter() throws Exception {
        Aggregations result = query(String.format("SELECT COUNT(*) FROM %s/account GROUP BY filter(gender='m'),gender", TestsConstants.TEST_INDEX));
    ...
    public void groupByOnNestedFieldWithFilterTest() throws Exception {
        Aggregations result = query(String.format("SELECT COUNT(*) FROM %s/nestedType GROUP BY  nested(message.info),filter('myFilter',message.info = 'a')", TestsConstants.TEST_INDEX));
        InternalNested nested = result.get("message.info@NESTED");
    ...
    public void minOnNestedField() throws Exception {
        Aggregations result = query(String.format("SELECT min(nested(message.dayOfWeek)) as minDays FROM %s/nestedType", TestsConstants.TEST_INDEX));
    ...
    public void sumOnNestedField() throws Exception {
        Aggregations result = query(String.format("SELECT sum(nested(message.dayOfWeek)) as sumDays FROM %s/nestedType", TestsConstants.TEST_INDEX));
        InternalNested nested = result.get("message.dayOfWeek@NESTED");
    ...
    public void histogramOnNestedField() throws Exception {
        Aggregations result = query(String.format("select count(*) from %s/nestedType group by histogram('field'='message.dayOfWeek','nested'='message','interval'='2' , 'alias' = 'someAlias' )", TestsConstants.TEST_INDEX));
    ...
    public void reverseToRootGroupByOnNestedFieldWithFilterTestWithReverseNestedAndEmptyPath() throws Exception {
        Aggregations result = query(String.format("SELECT COUNT(*) FROM %s/nestedType GROUP BY  nested(message.info),filter('myFilter',message.info = 'a'),reverse_nested(someField,'')", TestsConstants.TEST_INDEX));
        InternalNested nested = result.get("message.info@NESTED");
        InternalFilter filter = nested.getAggregations().get("myFilter@FILTER");
        Terms infos = filter.getAggregations().get("message.info");
        Assert.assertEquals(1,infos.getBuckets().size());
    ...
    public void reverseToRootGroupByOnNestedFieldWithFilterTestWithReverseNestedOnHistogram() throws Exception {
        Aggregations result = query(String.format("SELECT COUNT(*) FROM %s/nestedType GROUP BY  nested(message.info),filter('myFilter',message.info = 'a'),histogram('field'='myNum','reverse_nested'='','interval'='2' , 'alias' = 'someAlias' )", TestsConstants.TEST_INDEX));
        InternalNested nested = result.get("message.info@NESTED");
        InternalFilter filter = nested.getAggregations().get("myFilter@FILTER");
        Terms infos = filter.getAggregations().get("message.info");
        Assert.assertEquals(1,infos.getBuckets().size());
    ...
    public void reverseToRootGroupByOnNestedFieldWithFilterAndSumOnReverseNestedField() throws Exception {
        Aggregations result = query(String.format("SELECT sum(reverse_nested(myNum)) bla FROM %s/nestedType GROUP BY  nested(message.info),filter('myFilter',message.info = 'a')", TestsConstants.TEST_INDEX));
        InternalNested nested = result.get("message.info@NESTED");
        InternalFilter filter = nested.getAggregations().get("myFilter@FILTER");
        Terms infos = filter.getAggregations().get("message.info");
        Assert.assertEquals(1,infos.getBuckets().size());
    ...
    public void reverseAnotherNestedGroupByOnNestedFieldWithFilterTestWithReverseNestedNoPath() throws Exception {
        Aggregations result = query(String.format("SELECT COUNT(*) FROM %s/nestedType GROUP BY  nested(message.info),filter('myFilter',message.info = 'a'),reverse_nested(comment.data,'~comment')", TestsConstants.TEST_INDEX));
        InternalNested nested = result.get("message.info@NESTED");
        InternalFilter filter = nested.getAggregations().get("myFilter@FILTER");
        Terms infos = filter.getAggregations().get("message.info");
        Assert.assertEquals(1,infos.getBuckets().size());
    ...
    public void reverseAnotherNestedGroupByOnNestedFieldWithFilterTestWithReverseNestedOnHistogram() throws Exception {
        Aggregations result = query(String.format("SELECT COUNT(*) FROM %s/nestedType GROUP BY  nested(message.info),filter('myFilter',message.info = 'a'),histogram('field'='comment.likes','reverse_nested'='~comment','interval'='2' , 'alias' = 'someAlias' )", TestsConstants.TEST_INDEX));
        InternalNested nested = result.get("message.info@NESTED");
        InternalFilter filter = nested.getAggregations().get("myFilter@FILTER");
        Terms infos = filter.getAggregations().get("message.info");
        Assert.assertEquals(1,infos.getBuckets().size());
    ...
    public void reverseAnotherNestedGroupByOnNestedFieldWithFilterAndSumOnReverseNestedField() throws Exception {
        Aggregations result = query(String.format("SELECT sum(reverse_nested(comment.likes,'~comment')) bla FROM %s/nestedType GROUP BY  nested(message.info),filter('myFilter',message.info = 'a')", TestsConstants.TEST_INDEX));
        InternalNested nested = result.get("message.info@NESTED");
        InternalFilter filter = nested.getAggregations().get("myFilter@FILTER");
        Terms infos = filter.getAggregations().get("message.info");
        Assert.assertEquals(1,infos.getBuckets().size());
    ...
    public void docsReturnedTestWithoutDocsHint() throws Exception {
        String query = String.format("SELECT count(*) from %s/account", TestsConstants.TEST_INDEX);
    ...
    public void docsReturnedTestWithDocsHint() throws Exception {
        String query = String.format("SELECT /*! DOCS_WITH_AGGREGATION(10) */ count(*) from %s/account", TestsConstants.TEST_INDEX);
        SqlElasticSearchRequestBuilder searchRequestBuilder = getSearchRequestBuilder(query);
        SearchResponse response = (SearchResponse) searchRequestBuilder.get();

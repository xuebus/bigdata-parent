...
import com.zhbwang.bigdata.es.es4sql.domain.*;
...
import com.zhbwang.bigdata.es.es4sql.parse.ElasticSqlExprParser;
import com.zhbwang.bigdata.es.es4sql.parse.ScriptFilter;
import com.zhbwang.bigdata.es.es4sql.parse.SqlParser;
import com.zhbwang.bigdata.es.es4sql.query.maker.QueryMaker;
import com.zhbwang.bigdata.es.es4sql.query.multi.MultiQuerySelect;
...
public class SqlParserTests {
    ...
    public void joinConditionWithComplexObjectComparisonRightSide() throws SQLFeatureNotSupportedException, IOException, SqlParseException {
        String query = String.format("select c.name.firstname,c.parents.father , h.name,h.words from %s/gotCharacters c " +
                ...
                "where c.name.firstname='Daenerys'", TestsConstants.TEST_INDEX, TestsConstants.TEST_INDEX);
        JoinSelect joinSelect = parser.parseJoinSelect((SQLQueryExpr) queryToExpr(query));
        List<Condition> conditions = joinSelect.getConnectedConditions();
        Assert.assertNotNull(conditions);
        Assert.assertEquals(1, conditions.size());
    ...
    public void searchWithOdbcTimeFormatParse() throws SqlParseException {
        String query = String.format("SELECT insert_time FROM %s/odbc WHERE insert_time < {ts '2015-03-15 00:00:00.000'}", TestsConstants.TEST_INDEX);
    ...
    public void innerQueryTest() throws SqlParseException {
        String query = String.format("select * from %s/dog where holdersName IN (select firstname from %s/account where firstname = 'eliran')", TestsConstants.TEST_INDEX, TestsConstants.TEST_INDEX);
        SQLExpr sqlExpr = queryToExpr(query);
        Select select = parser.parseSelect((SQLQueryExpr) sqlExpr);
    ...
    public void inTermsSubQueryTest() throws SqlParseException {
        String query = String.format("select * from %s/dog where holdersName = IN_TERMS (select firstname from %s/account where firstname = 'eliran')", TestsConstants.TEST_INDEX, TestsConstants.TEST_INDEX);
        SQLExpr sqlExpr = queryToExpr(query);
        Select select = parser.parseSelect((SQLQueryExpr) sqlExpr);
        Assert.assertTrue(select.containsSubQueries());
        Assert.assertEquals(1, select.getSubQueries().size());
    ...
    public void innerQueryTestTwoQueries() throws SqlParseException {
        String query = String.format("select * from %s/dog where holdersName IN (select firstname from %s/account where firstname = 'eliran') and age IN (select name.ofHisName from %s/gotCharacters) ", TestsConstants.TEST_INDEX, TestsConstants.TEST_INDEX, TestsConstants.TEST_INDEX);
        SQLExpr sqlExpr = queryToExpr(query);
        Select select = parser.parseSelect((SQLQueryExpr) sqlExpr);
        Assert.assertTrue(select.containsSubQueries());
    ...
    public void parseJoinWithOneTableOrderByAttachToCorrectTable() throws SqlParseException {
        String query = String.format("select c.name.firstname , d.words from %s/gotCharacters c " +
                ...
                , TestsConstants.TEST_INDEX, TestsConstants.TEST_INDEX);
